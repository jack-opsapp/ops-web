...
The third customer's misbound thread was held for separate migration approval, then completed as recorded below. The runtime correction itself needs no new database schema.
...
## Exact thread finalization

Migration `20260914211750_finalize_exact_email_thread_recovery.sql` adds a private immutable ledger and service-role-only finalizer. It requires an authorized actor, an owned active mailbox lease, current full source/target/thread/link snapshots, a complete exact message set, and finalized per-message recovery receipts. It independently checks current attachment scans, inspection, materialization, and the unchanged attachment ID sets. The ordinary same-client reassignment API remains unchanged. The source filename matches the actual Supabase migration version; its SQL is unchanged from the tested preparation.
...
The private four-message rehearsal pinned original message authorship, bodies/dates, protected source state, ten stored attachment copies and ten separate older discovery rows. Repeated read-only provider responses confirmed rotating download tokens despite stable MIME parts. The repair therefore requires exact stored-file replay and explicit attachment accounting; a fresh provider scan alone cannot establish completion for that history.

The private stored-file replay uses the existing canonical attribution, inspection and exact scan contracts with the original stored descriptors. It checks current MIME parts and stored byte hashes, forbids provider downloads and object writes, preserves all original IDs, and separately accounts for unchanged discovery rows. Seventeen focused tests passed independently, including quoted files, zero-file scans, interrupted inspection, duplicate/changed evidence, and replay. The final integrated read-only rehearsal passed against the current four-message provider snapshot and all ten stored copies at 21:01:24 UTC. The runner and adapter had zero scoped TypeScript diagnostics; imported application dependencies retained the same 21 baseline diagnostics.

## Approved production finalization

After explicit migration and exact-recovery approval, Supabase applied migration version `20260914211750`. Independent catalog readback confirmed the private ledger has RLS and no application-role table privileges; only service_role can execute the finalizer. Both existing ordinary reassignment function definitions retain their exact pre-migration hashes.

The initial live replay stopped before finalization because the private checker expected attachment URLs without filenames. The actual deployed `refresh_email_activity_attachments` function correctly includes a sanitized filename. The synthetic repository fixture was corrected to reproduce that database contract, producing three failing tests before the checker fix. Nineteen focused tests then passed, including controls rejecting wrong attachment IDs or filenames. No production application or database contract was changed to bypass this check.

Exact recovery finalized at **2026-09-14 21:21:34.610228 UTC**. Immediate replay returned `already_finalized:true`. Independent production readback and original evidence comparison verified:

- Four original activities and four projected correspondence events belong to the single correct new lead. Authors, recipients, bodies, dates and identities are preserved; both thread projections point to it.
- All ten original stored attachment payloads remain unchanged and correctly attributed. A separate storage read verified all ten file hashes and **1,857,278 bytes**. All ten older discovery records remain completely unchanged.
- All four exact scans and message recovery receipts are complete. Five customer image inspections completed; five operator-sent copies retain the expected operator_sent skip outcome.
- The source's completed status, manual boundary, client/project, assignment, images and original job thread remain intact. Correspondence counts now correctly exclude the four moved emails. Both summaries were regenerated from their respective histories.
- No additional client or project was created; one target lead and one finalization receipt exist. The mailbox is active, sync enabled, and unlocked. The only leads created since the prior audit are the two deliberately recovered inquiries.

This establishes the audited repair in production, not universal future classification accuracy. All customer-specific manifests, source emails and readbacks remain private.
