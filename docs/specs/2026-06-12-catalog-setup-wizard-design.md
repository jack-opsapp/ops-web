...
| **Upload a spreadsheet (CSV/XLSX)** | deterministic mapper | port iOS `CatalogCSVMapper` (family-grouping, header alias auto-map, name→category/unit resolution) → cards | **CSV web port BUILT 2026-06-14** — `buildUploadCards` auto-routes a clean CSV to the products/stock mapper → show-diff dedupe-bind (re-import re-syncs, never doubles) → stages onto the canvas (the canvas IS the review surface, so no separate column-map step); `UploadPane` drop/browse UI. Building blocks committed (`29acc34b`/`2e7f6fff`); route/shell/i18n wiring held (parallel-session commingle). **XLSX deferred** — the `xlsx` pkg would mutate the shared symlinked `node_modules` (`xlsx-parse.ts` stays a stub); pkg-install decision flagged to Jackson, ship-CSV-first. Stock-row dedupe also deferred (variant-id vs family-id UPSERT landmine). |
