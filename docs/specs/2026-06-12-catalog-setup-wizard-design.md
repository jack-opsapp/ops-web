...
| **Upload a spreadsheet (CSV/XLSX)** | deterministic mapper | port iOS `CatalogCSVMapper` (family-grouping, header alias auto-map, name→category/unit resolution) → cards | **CSV + XLSX web port BUILT 2026-06-14/15** — `buildUploadCards` auto-routes a clean CSV/XLSX to the products/stock mapper → show-diff dedupe-bind (re-import re-syncs, never doubles) → stages onto the canvas (the canvas IS the review surface, so no separate column-map step); `UploadPane` drop/browse UI. **XLSX (Jackson-approved 2026-06-15):** `xlsx@0.18.5` (SheetJS) installed purely-additively into the shared node_modules; `parseXlsx` lazy-loads it via dynamic import (~900KB stays out of the main bundle, loads only on .xlsx drop). Committed: building blocks (`29acc34b`/`2e7f6fff`), xlsx (`e81880e2`), E2E harness (`8fa71dbc`); E2E spec held (parallel-session commingle with the template test). **Stock-row dedupe still deferred** (variant-id vs family-id UPSERT landmine). |
