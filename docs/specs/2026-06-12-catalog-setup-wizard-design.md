...
|--------|------|--------------|--------|
| **Connect QuickBooks** | structured pull | OAuth (reuse existing connect) → read-only pull of QB `Item` → mapped cards | connect EXISTS; pull→map→dedupe **BUILT** (read-only, gated to Canpro/flag; QB inventory → priced material with on-hand surfaced; stock-row import deferred) |
| **Upload a spreadsheet (CSV/XLSX)** | deterministic mapper | port iOS `CatalogCSVMapper` (family-grouping, header alias auto-map, name→category/unit resolution) → cards | **CSV + XLSX web port BUILT 2026-06-14/15** — `buildUploadCards` auto-routes a clean CSV/XLSX to the products/stock mapper → show-diff dedupe-bind (re-import re-syncs, never doubles) → stages onto the canvas (the canvas IS the review surface, so no separate column-map step); `UploadPane` drop/browse UI. **XLSX (Jackson-approved 2026-06-15):** SheetJS lazy-loaded via dynamic `import("xlsx")` (~900KB stays out of the main bundle, loads only on .xlsx drop); `parseXlsx` uses `XLSX.read({type:'array'})` + `sheet_to_json({header:1,raw:false,defval:'',blankrows:true})`. **CVE remediation 2026-06-15:** migrated off the deprecated `xlsx@0.18.5` npm build (prototype-pollution/ReDoS advisories, no fix on npm) to the maintained `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz` pin — package.json + the branch package-lock.json (integrity + the 8 orphan SheetJS transitive deps pruned → merge-clean for `npm ci`/Vercel); 0.20.3's ESM build exports `read`/`utils` as named exports so `parseXlsx` is unchanged. The shared symlinked node_modules locally still physically holds 0.18.5 (cannot be safely swapped mid-parallel-session — flagged); the lockfile is the source of truth and a real deploy resolves 0.20.3. Committed: building blocks (`29acc34b`/`2e7f6fff`), xlsx (`e81880e2`), E2E harness (`8fa71dbc`). **Stock-row dedupe still deferred** (variant-id vs family-id UPSERT landmine). |
...
- **Two turns, not a walkthrough (Jackson 2026-06-13 — the whole point of agent mode).** Agent mode is the fast lane: the owner answers **one or two plain questions** ("what do you do, and how do you charge?"), then the agent generates the **entire** operating system onto the canvas at once. The owner skims, fixes anything off (inline or the item editor), and hits **build it**. Agent mode does NOT step the owner through SELL → STOCK → TYPES module-by-module. The multi-step module walkthrough (survey → SELL → STOCK → TYPES → REVIEW) is the **deterministic fallback only** — no agent / offline / "I'll do it myself." Minimal effort, conversational, done in a beat.
- **The module rail has two roles.** In **agent mode** it is a glance-able overview of what was generated (per-section counts + jump-to a section to review) — never a gate, never a forced sequence. In the **deterministic path** it is the actual stepper the owner advances. Same canvas, same `build it`; the agent just collapses the steps into a short chat. (Implementation: the Phase-1 step-machine drives the deterministic walkthrough; agent mode populates all sections up front and treats the rail as navigation.)
...
- **Dedupe:** match imported rows against the live catalog on `lower(trim(sku))` (and name when SKU is absent) → a matched card defaults to **show-diff** (per-field accept; Jackson 2026-06-13), with merge-all and skip also offered, instead of a create that would hard-fail the DB unique index. Add nullable `external_source` + `external_id` columns (additive, iOS-safe) so re-imports re-sync instead of duplicating (avoids the won-conversion class of bug). **Implemented (QB lane):** `catalog_setup_save` only `ON CONFLICT (id)` and never touches `external_*`, so (a) import-time `matchCards` binds each re-pulled match to its live row id (the commit UPSERTs by id — an unmatched dup would otherwise hard-reject on `uniq_products_sku_per_company` / `uniq_products_external_id_per_company`), and (b) a post-commit stamp (`external-identity-stamp.ts`) writes `external_source`/`external_id` onto the committed rows so the NEXT pull re-matches by `external_id` first — surviving a rename or sku drift. **Per-field show-diff UI BUILT 2026-06-15/16:** a duplicate (merge) card renders a verdict toggle per changed field — TAKE INCOMING (overwrite the live row) vs KEEP ON FILE (the live value stands) — defaulting to take-all (doing nothing = the prior whole-card merge). The verdict lives on `StagingCard.fieldSelections` (`SET_FIELD_SELECTION`). **Merge preservation (the key correctness property):** `catalog_setup_save` UPSERTs EVERY product column from the doc (absent ⇒ a default: `is_active→true`, `description→null`), so a merge that sent only the incoming card fields would wipe descriptions/categories/storefront and reactivate a retired product. The commit adapter therefore REBUILDS a merge doc FROM the on-file row (full committable column set threaded as `existing-rows.OnFileProduct` for EVERY matched card via the client→commit-body→route hand-off) and overrides ONLY the accepted verdicts — so "only accepted fields overwrite the live row" holds and "the rest stays on file" is literally true (`is_active` carried from on-file; an EDITED matched card keeps its upsert binding; a stale merge whose live row vanished falls back to create). Only the fields the RPC overwrites get a toggle — **name, base_price, is_taxable**; `unit_cost` is excluded from the diff because the RPC never writes `products.unit_cost`, so a merge never changes cost (the on-file cost stands) — and a newly CREATED product's cost is persisted by the post-commit `cost-stamp.ts` (the same RPC-ignores-this-column pattern as external_id). Bulk shortcuts are symmetric/non-destructive: KEEP ON FILE (every verdict → keep — the card stays) / TAKE INCOMING (`MERGE_CARD` — overwrite every field); the card's × REJECT is the only drop. The pure matcher-lane resolver (`applyDedupe` + `CardResolution.fieldSelections`) is retained as the equivalent primitive for the `matchCards`→`applyDedupe` path.
