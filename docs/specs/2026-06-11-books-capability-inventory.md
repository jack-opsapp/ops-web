...
1. **MetricsHeader presentation tier (E1/I1/A1)** — the per-tab MetricsHeader's sparklines, trend arrows, and card-flip breakdown *presentation* did not carry into direction A (plan D5: the heavy header is replaced by the ledger strip + quiet stat line). **All metric data carries over** — post-review, every inventoried number (incl. estimates sent/mo + avg, invoices collection rate + avg days-to-pay, accounting breakdown counts) renders in the stat lines; only the per-metric mini-viz/flip chrome is gone. Sign-off requested on the presentation drop.
2. **`accounting.view`-only persona sees empty financial data** — pre-existing, carried over: invoice/payment RLS and the `useInvoices` hook gate require `invoices.view`, so a (today nonexistent — all prod roles holding `accounting.view` also hold `invoices.view`) custom grant of only `accounting.view` would reach the A/R view and ledger strip but read zero rows, exactly as on the retired `/accounting` page. Serving that persona needs an RLS/permission-model change — out of P3.1 scope; flagged for a future wave.
3. Otherwise **none**: all capabilities E1–A8 carry over. The only intentional non-carries are the `/money/cashflow` placeholder *content* (replaced by redirect; full web forecast separately tracked) and dead code E15/I13.

## 9 · Wave review outcome (2026-06-11)

Adversarial multi-agent audit (4 auditors × per-finding refutation): 33 findings raised, 28 confirmed, 5 refuted. All 28 confirmed findings remediated in-wave except the two §8 items above (sign-off / future-wave). Notable correctness fixes: local-date period boundaries (UTC serialization shifted windows a day), date-based A/R drill (`status=overdue` virtual filter — the past_due enum hid ~$18k of date-overdue balance in prod), hydration-safe segment default. Full design-token (11px floor, brick-as-text, monochrome provider icons, token-traced viz colors, reduced-motion) and i18n (status tags, modals, drill chips, payment methods, es quality) conformance passes applied.
