...
| `duties` | `computeDuties` | `hygiene` every run; `creative` when an enabled engine ad group has an enabled `role-control` ad first seen ≥ 28 days ago, no running test and no challenger in place (`challengerInPlace`: an enabled `role-challenger` ad, or one the guardrail holds for a landing page — a group tests one challenger at a time, so a pair waiting for its first shared day or for a decision on its verdict is never named; 2026-09-14); `structure` on the first released run of the Vancouver month; `bidding_ladder` when trial starts hit a ladder trigger (≥ 15 in each of the last two 30-day windows, or ≥ 30 in the last 30) |
| `duty_notes` | `computeDuties` (+ open `ads_guardrail_pauses`) | one sentence per duty saying why it is due (names the ad groups for `creative`). An ad group whose ad the disapproved-ad guardrail paused for a copy verdict is named at once, whatever its control's age, as `<group> (Google disapproved an ad for <reason>; write its replacement)` — only when `replacementDue` holds (serving engine campaign and group, an enabled control other than the paused ad, no running test, no other challenger in place, not a landing-page-only verdict), the same predicate that lets the `AD DISAPPROVED` alert promise a replacement (2026-09-11) |
...
| `snapshot` | `ads_entities` → `mapEntitySnapshot` + the committed blueprint (`copy-kinds.ts`) | campaigns (kind, labels, budget, bidding, CPC ceiling), ad groups (landing page, `copyKind`), ads (role, approval, RSA assets with pins, paths), keywords, shared negative lists with members and attachments, campaign negatives, labels. A campaign's `kind` is the blueprint's for the campaign of that exact name (`legacy` by label; `other` when the blueprint does not declare it — a name grants nothing). An ad group's `copyKind` is the rules its ads answer to: the blueprint group's `copyKind`, else its campaign's kind, `competitor` only when its ads land on a `/compare/` page, `core` under a campaign with no kind. Only a `competitor` group may name a competitor (v2, 2026-09-14; v1 read kinds off `BRAND`/`CORE`/`COMPETITOR` name prefixes, so the phase 2 competitor-intent groups looked core) |
...
| `tests` | `ads_tests` | running and concluded tests with OPS's `stats` (impressions, clicks, CTR per arm, z, p) and `state`. Includes the tests the worker opens for pairs nobody proposed (the blueprint's, an engine-built ad group's), started on the first day both ads served, and `cancelled` tests with `stats.reason` when an ad left the pair (2026-09-14) |
