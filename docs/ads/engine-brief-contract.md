...
| `duty_notes` | `computeDuties` (+ open `ads_guardrail_pauses`) | one sentence per duty saying why it is due (names the ad groups for `creative`). An ad group whose ad the disapproved-ad guardrail paused for a copy verdict is named at once, whatever its control's age, as `<group> (Google disapproved an ad for <reason>; write its replacement)` — only when `replacementDue` holds (serving engine campaign and group, an enabled control other than the paused ad, no running test, not a landing-page-only verdict), the same predicate that lets the `AD DISAPPROVED` alert promise a replacement (2026-09-11) |
