...
### 4.7 Performance
Company filter first (`company_id = private.get_user_company_id()` on every base table), then RLS, then token predicates. Trigram GIN indexes serve `like '%tok%'` on normalized title/address/name for tokens ≥ 3 characters; shorter tokens scan the company's rows (hundreds). Target: p95 under 150 ms for a 3-token query on the largest company. The executor records `explain (analyze, buffers)` for one representative query in the task report.
