...
**Status:** Approved for local implementation by Jackson on 2026-08-22 and amended the same day to include authoritative deck-design geometry and measurements. This approval covers the isolated branch build only; it does not authorize a database apply, push, deployment, OAuth grant change, production exposure, or any write capability.
...
The existing eleven production reads remain unchanged. Twenty-three purpose-built reads add customer context, work management, site evidence, authoritative deck-design geometry, sales documents, payments, expenses, supply, company context, team context, integration health, and an operational overview. The resulting catalogue contains thirty-four read-only tools. Every write capability, including all existing prepare/commit shells, remains unavailable and externally disabled.
...
Reserve the exact identity `2026-08-22.capability-manifest.v8` before candidate work so every isolated policy, authorization, repository, and v8-only RPC is tested against its final manifest identity. The reserved identity cannot enter the active manifest, runtime dispatch, OAuth grantability, or exposure catalogue. Finalize and test all twenty-three P2 contracts and implementations, then mint the immutable aggregate once by copying the already-tested candidate policy bytes without semantic reminting. V8 contains the existing eleven reads and the complete twenty-three-read P2 policy catalogue. Existing v7 grants and callers continue through exact v7/v8 compatibility bridges; new RPCs accept the reserved/final v8 identity only.

Rollout state is separated from proof/policy identity. Refactor external registration into a small immutable, server-owned `MCP_EXPOSURE_CATALOG` with its own revision. `2026-08-22.mcp-exposure.v1` contains exactly the eleven production reads and their seven currently grantable OAuth scopes. One fail-closed composition-root resolver returns the exact active immutable exposure object—revision, tool IDs, and grantable scopes together—and that same frozen object is injected into tool registration, DCR, authorization, token issuance, and refresh validation. Unknown revisions, duplicate IDs/scopes, or any tool/scope mismatch fail startup. A later, separately authorized exposure-catalogue revision is a new constant that may add one proven capability or coherent wave plus only its required grantable scopes; an existing revision is never edited in place. The server rejects an exposure entry unless the manifest says `operation=read`, a concrete domain mapping exists, the capability is implemented, its complete OAuth labels exist in that exposure revision, and the active server configuration names that exact revision. Writes and unimplemented reads can never enter the exposure catalogue.

This avoids repeated SQL reproof bridges while preserving a capability-sized rollout and rollback switch. V8 application code can therefore contain all twenty-three implemented P2 reads while the production host still sees exactly eleven.
...
Exact visit context with booking/lead identity, state, checklist completion summary, artifact counts by safe kind/review inclusion, bounded timeline facts, and opaque references for every active deck design explicitly bridged to the visit. Closed, explicitly requested sections may return bounded visit notes, measurements, and checklist answer values as untrusted business data. It excludes deck geometry itself, internal notes, identity drafts, calendar/provider identifiers, attendee JSON, raw annotation state, and storage URLs.

#### `get_deck_design_geometry`

Exact deck-design geometry and measurements. Discovery occurs through `get_site_visit_context` or `list_job_artifacts`; those tools return every opaque value required by one strict input union: `{ source: "job_artifact", job_ref: { kind: "opportunity" | "project", id }, deck_design_ref }` or `{ source: "site_visit_artifact", site_visit_ref, deck_design_ref }`. The caller cannot select linked/unlinked authority; the server derives the exact visit variant. The tool does not search by customer, title, filename, or arbitrary drawing field. A site-visit source follows the explicit active `site_visit_artifacts.deck_design_id` bridge and separately proves both current visit authority and the linked design's current company/job authority; historical opportunity-ID equality is not assumed.

The public projection contains bounded design metadata, schema/calculation revision, measurement quality, renderable two-dimensional topology, levels, local vertex references, edge segments and boundary roles, directed surface outer loops and holes, and bounded level/stair connections. Coordinates are a normalized local drawing projection; stored real-world edge dimensions remain separately identified as measurement facts. The v1 area formula is the DeckKit closed-face area across every level—outer loop minus holes—divided by the squared effective scale and by 144. The v1 flat/parapet guard formula uses only finite positive `deck_edge` dimensions with a railing configuration and subtracts the exact stair opening plus `36` inches for each `isGate` item before clamping at zero. The v1 stair formula uses both sides of the DeckKit stringer. Edge-stair total rise first uses a finite positive persisted `totalRiseInches`; only when that is absent does it resolve owning-surface absolute elevation, then the midpoint of endpoint elevations resolved vertex, level, and overall. A finite positive explicit tread count wins, otherwise DeckKit calculates it from `totalRise` and the current positive `risePerStep`; stringer run uses the current positive `runPerTread`. Level connections use the positive difference between their exact upper/lower elevations with the connection's exact tread/run configuration. Flat, stair, non-parapet, parapet, and combined guard totals are summed in full-precision inches and rounded only at the final public boundary; canvas distance is never substituted silently.

Measurement quality is per metric. Area depends only on finite valid topology and a finite positive scale. Flat and parapet railing each depend only on every configured contributing edge's finite positive stored dimension, gate count, and stair opening. Edge- and level-stair railing depend only on their own finite positive rise, tread count, and run. The combined guard total is authoritative only when every included rail subtype is authoritative; one unavailable subtype never silently disappears. A missing/stale rail dimension or stair rise does not suppress valid deck area, and invalid area topology does not suppress otherwise authoritative railing. Coordinates may be negative and require only finiteness; positivity applies only to scale, authoritative dimensions, openings, rises, tread counts, and runs. Unknown schema or invalid references fail the geometry projection; otherwise each metric returns its own fixed authoritative, unavailable, or invalid state and warning.

The private repository projection may contain only the bounded geometry fields required by the versioned calculator. The public result never contains raw `drawing_data`, persisted `components`, catalog/product identifiers, prices or costs, assigned-item payloads, editor recovery/future blocks, framing, terrain, footings, house openings, photo overlays, permit/zoning/compliance payloads, storage/provider paths, creator identity, or private notes. Safe closed semantic fields such as `house|wall|open`, railing kind, and surface/house finish family may be returned only as untrusted design data and never as a catalog or price claim. Binary previews remain separately authorized, single-use evidence resources.

The calculator is a versioned pure TypeScript port locked against fixtures produced independently by current OPS iOS and standalone DeckKit for single- and multi-level plans, adjacent surfaces, holes, gates, stairs, parapets, self-intersections, stale/missing dimensions, and legacy schemas. Stored `components` is a cross-check only, not the measurement authority: when present, it is recomputed and compared component-by-component using DeckKit's exact per-edge/per-surface two-decimal persistence rounding, never by comparing its summed rounded values with the final-only-rounded authoritative total. A recognized legacy drawing without components receives a fixed witness-unavailable warning; a present mismatch fails closed.

Source bytes are rejected above 1 MiB. Per-collection checks then reject more than 16 planes, 160 vertices, 240 edges, 64 surfaces, 32 connections, or 320 directed boundary references. After duplicate/reference/finite-value validation, `topology_units = planes + vertices + edges + surfaces + connections + directed_boundary_references` must be at most 500; the individual ceilings do not imply they may coexist at their maxima. Finally the exact MCP serializer must remain at or below 60,000 characters. Rejection occurs in that order and never truncates. Generated worst-case fixtures at every individual ceiling, at exactly 500 topology units, at 501, and at the serializer boundary prove the accepted envelope.

Local references use immutable `deck-local-ref:2026-08-22.v1`: planes are ordered by level sort order then source ID; vertices, edges, surfaces, and connections are ordered by their source IDs within the plane and exposed only as compact ordinal references. Every response carries an opaque `geometry_source_fence` bound to actor, company, grant, design, canonical content hash, selected anchor, exact source-revision vector, calculator revision, and local-ref revision. The fence and references are valid only for that exact source snapshot and form the non-authorizing stale-write input for a future prepare operation.

The geometry source fence is invalidated literally, not heuristically. Any selected `deck_designs` change to tenant or job parent, title, drawing JSON, version, timestamps, or deleted state advances the `deck_designs` revision for every distinct OLD/NEW company. Any canonical `site_visit_artifacts` deck-bridge insert, delete, reparent, active-state change, or selected identity/review/timestamp change advances `artifacts`, `site_visits`, and `deck_designs` for every distinct OLD/NEW company. Selected visit authority/identity changes advance `site_visits`. The RPC also binds the existing job revision, but a design-parent change does not advance that legacy counter or alter v6/v7 cursor behavior; it invalidates through `deck_designs` and the relationship is re-proved in the same statement. Runtime tests pin every field and prove that unrelated columns do not create false churn.
...
- Deck designs: `ops.files.read`, with no `photos.view` substitution, plus `ops.jobs.read` and `deck_builder.view=assigned` with exact `projects.view=all|assigned` or `pipeline.view=all|assigned` when the design has a current job parent. A site-visit anchor additionally requires `ops.site_visits.read`, the exact visit permission variant, and the active artifact bridge; the bridge never manufactures assigned deck authority. A parentless design always requires `deck_builder.view=all`. A genuinely unlinked visit with a parentless design also requires `pipeline.view=all`; there is no assigned variant. The generic artifact read returns only the opaque design reference; geometry is available only through `get_deck_design_geometry`.
...
The generic entity authorizer remains limited to the entity kinds it currently understands. Estimate, invoice, payment, expense, artifact, deck design, catalogue, company, team, and integration reads each use a dedicated nominal authorizer and fixed same-statement RPC proof; implementations may not translate the table name into a generic entity claim.
...
Every non-mutating public read RPC is `STABLE SECURITY DEFINER` only when privileged access is necessary, has a fixed `search_path`, explicitly validates the service-role request boundary, revokes `PUBLIC`, `anon`, and `authenticated`, and grants only `service_role`. The durable limiter consume RPC and single-use evidence redemption RPC are explicitly `VOLATILE SECURITY DEFINER` because they atomically mutate security bookkeeping; they retain the same fixed search path, service-role guard, and ACL boundary. Private helpers remain non-executable by application roles unless an expression index requires a deliberately audited immutable helper grant to roles that already write the indexed table.
...
First split the monolithic read registry into behavior-identical domain modules, add the bounded P2 read/proof/cursor kernel alongside the live implementations, add the durable limiter and domain source revisions, fix OAuth consent-catalogue/scope-ceiling behavior, freeze the binary artifact identity/resource-link/redemption contract, and finalize every P2 candidate contract against the reserved v8 identity without activating, aggregating, granting, dispatching, or exposing it. Implement customer context and tasks against the internal candidate-policy set. The active manifest remains v7 and the initial exposure-catalogue revision remains the exact production eleven, so every P2 read remains absent from the host.

### Wave B — money

Implement job artifacts and evidence redemption first, then the two existing site-visit reads against that frozen artifact boundary. Implement `get_deck_design_geometry` after site-visit and artifact identity are stable, with its own deck-design source revision, fixed same-statement RPC, bounded topology parser, and DeckKit parity fixtures. Implement sales documents, then payments, then expenses and reimbursements as separate revision domains. Implement the bounded work queue only after those source projections are stable, so its financial and expense attention cards use the same proven contracts. Money reads require the strict shared money representation, currency/source-validity proofs, and independent full-scope tests.
...
Implement operational overview only after every source-domain private projection is stable. Once all twenty-three implementations and candidate policies are final and green, mint v8 exactly once and finalize its compatibility migration. Then run the full thirty-four-tool v8 manifest/domain matrix, the exact eleven-tool initial exposure matrix, database runtime matrix, adversarial privacy suite, v6/v7 continuity suite, and real host workflows. The deck acceptance chain must prove customer discovery → visible job → site visit → exact deck reference → verified measurements and renderable geometry, with zero writes. A separately authorized release may then publish a new immutable exposure-catalogue revision containing only the proven capabilities approved for that release. V8 itself is never mutated, and neither deployment nor a manifest change silently expands the host's tool set or grantable scopes.
...
## 13. Approved future write direction — not part of P2

The read catalogue deliberately creates the semantic and proof foundation for two future write families, but neither is implemented, registered, grantable, or externally visible in manifest v8.

### 13.1 Deck design creation and editing

Future MCP writes may create or edit a deck from an owner instruction or a customer-supplied drawing only through a generic durable prepare/approve/commit coordinator. A host may interpret image labels such as `PICKETS`, `GLASS`, or `PRIVACY`, but that interpretation is untrusted proposal data. OPS—not the host—must validate finite geometry, topology, scale, authoritative dimensions, surface/edge identities, house/open boundary roles, catalogue eligibility, and the exact source/version fence.

The prepare operation returns a complete visual and numeric preview: normalized geometry, calculated area/railing totals, exact surfaces and edges being changed, catalogue products/options selected, unmatched or ambiguous markings, source warnings, and the before/after diff. It cannot claim real measurements without a valid scale or dimension anchor, cannot infer an uncertain house edge, cannot silently substitute a catalogue item, and cannot produce a commit receipt while any required interpretation remains ambiguous.

The operator must explicitly approve that exact preview. A server-issued one-time receipt then permits one idempotent commit that re-authorizes the actor/grant/company/job, rechecks the unchanged design and catalogue revisions, applies one atomic semantic mutation, records an immutable privacy-safe change-set audit, and returns the new exact readback. The host never writes raw `drawing_data`, table rows, arbitrary JSON paths, prices, or storage fields. Creation, geometry editing, surface finish assignment, house cladding assignment, and railing assignment are separate closed change operations even when one preview composes them.

### 13.2 Catalogue setup from host knowledge

A future catalogue-setup write may let Claude or ChatGPT propose OPS products, options, units, pricing, assemblies, supplier facts, and mappings based on the user's existing host history. Host memory is never treated as verified company authority. The prepare result must show the complete normalized catalogue proposal, currency/tax/unit assumptions, duplicates, conflicts, omissions, unsupported prices, source confidence, and every row that would be created, changed, archived, or left untouched.

Only explicit owner approval of that exact diff can mint a one-time idempotent commit receipt. Commit rechecks current catalogue and permission revisions, rejects stale or ambiguous mappings, never imports secrets or private conversation history, and readbacks the resulting catalogue through the same bounded MCP contracts. This is a future CRUD programme after the complete read catalogue and generic change-set coordinator are proven; it is not a shortcut around them.
