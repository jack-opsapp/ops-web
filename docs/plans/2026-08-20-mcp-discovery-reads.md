...
**Execution status (2026-08-21):** Database-first rollout is complete; code rollout is still pending. Local `main` contains the complete eleven-read implementation, external exposure, production-portable `pg_trgm` operator-class resolution, ledger alignment, indexed-writer ACL repair, and the manual-sync failure contract through commit `c2f302f9`. Production recorded discovery migration `20260822015049_agent_discovery_reads_20260820220000` and repair migration `20260822015939_agent_discovery_index_writer_acl_20260822015828`. Readback proves both discovery RPCs are service-role-only, all 40 indexes are valid/ready, the six trigram indexes bind the installed `public.gin_trgm_ops`, and all existing boundaries accept the zero-downtime v6/v7 bridge. A post-apply email write exposed missing execution rights for the private expression-index helper graph; the repair grants only the exact eight transitive index helpers to the three roles that already write the indexed tables, keeps `PUBLIC` and every non-index discovery helper denied, and was verified with rolled-back role-specific writes plus independent review. The next scheduled sync proved the original ACL failure cleared without advancing or manually altering the protected cursor. Current migration SHA-256 is `2f91ba90b01e3249180e30db0477dde1acffcc1beff1b74c6f71b9bd850570e8`; repair SHA-256 is `e27fff38ad2ad22f3a47b6cf5223be585750e7c85ba0b0b2696179e0a459ac1f`. Fresh local gates include migration static 32/32, both manual-sync routes plus authorization 67/67, Node TypeScript clean, Prettier clean, PostgreSQL 17 fresh-install convergence, and independent P0/P1 review none. The production app still exposes the prior nine-read surface because this local `main` has not yet been pushed or deployed. Safe remaining order is code push, exact deployment verification, eleven-tool canaries/audit readback, then Bible closeout; v6 removal remains a separate later change only after old work drains.
