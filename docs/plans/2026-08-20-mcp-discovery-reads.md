...
**Execution status (2026-08-21):** Database and code rollouts are complete. Feature release commit `5eb4b561c14cf8dace2b906d50ebcd34a0ba13db` was served by READY Vercel deployment `dpl_6ZMAhdXuweX9jSuVG8T58Z5gzWko` at `app.opsapp.co` and exposes exactly eleven read-only MCP tools, including `search_customers` and `search_jobs`. Production recorded discovery migration `20260822015049_agent_discovery_reads_20260820220000` and repair migration `20260822015939_agent_discovery_index_writer_acl_20260822015828`. Readback proves both discovery RPCs are service-role-only, all 40 indexes are valid/ready, the six trigram indexes bind the installed `public.gin_trgm_ops`, and all existing boundaries accept the zero-downtime v6/v7 bridge. A post-apply email write exposed missing execution rights for the private expression-index helper graph; the repair grants only the exact eight transitive index helpers to the three roles that already write the indexed tables, keeps `PUBLIC` and every non-index discovery helper denied, and was verified with rolled-back role-specific writes plus independent review. The next scheduled sync proved the original ACL failure cleared without advancing or manually altering the protected cursor. Current migration SHA-256 is `2f91ba90b01e3249180e30db0477dde1acffcc1beff1b74c6f71b9bd850570e8`; repair SHA-256 is `e27fff38ad2ad22f3a47b6cf5223be585750e7c85ba0b0b2696179e0a459ac1f`. Fresh local gates include 83 files / 2,023 tests green, Node 22 TypeScript clean, Prettier clean, PostgreSQL 17 fresh-install convergence, and independent P0/P1 review none. A live authenticated production canary listed all eleven tools; nine returned successful results, while conversation context and correspondence evidence returned their expected privacy-safe `NOT_FOUND` outcomes for a job without those records. The canary grant was revoked, the next bearer request was rejected with `401`, all disposable OAuth rows were deleted with zero-row readback, and the immutable request audit remains. All write and site-visit capabilities remain dark. The OPS Software Bible carries the matching production record; v6 removal remains a separate later change only after old work drains.
