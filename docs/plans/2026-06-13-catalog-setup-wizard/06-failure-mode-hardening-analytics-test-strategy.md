...
> **Implementation status — 2026-06-14 (branch `feat/catalog-setup-wizard`, LOCAL, no push).**
> **Landed:** 6.1 analytics `company_id` migration (APPLIED to prod), 6.2 prerequisite predicates, 6.3 session-lock — pure predicate (`isHeldByOther`/`buildSessionId`) + **DB-substrate service/heartbeat NOW WIRED** (2026-06-14, commit `14f0984f`): `session-lock-service.ts` (fail-open probe/acquire/heartbeat/release over a swappable `LockStore`), `use-catalog-setup-lock.ts` (acquire + 30s heartbeat + release hook), route renders the `session_locked` GatePanel; **substrate `catalog_setup_session_locks` APPLIED to prod 2026-06-15** (Jackson: "apply + turn on" — dedicated table chosen over wizard_states/save_requests; company_id PK, anon+authenticated `private.get_user_company_id()` RLS) and the **lock is ON BY DEFAULT** (`NEXT_PUBLIC_CATALOG_SETUP_LOCK_ENABLED="false"` is the kill-switch); 6.4 compound per-step gates (built + wired into the route as the StepContext/entry source), 6.7 online-status hook + fallback resolver (prior batch), 6.8 `wizard_analytics` builder/dispatcher + `use-catalog-setup-analytics` hook (wired: shown/started/step_completed/skipped/abandoned/completed), 6.11 inventory-off prompt (+ `DOWNSHIFT_STOCK_TO_PRODUCTS` reducer + `set_company_inventory_mode` mutation) and the BUILD-IT disabled-with-reason (already in the shell). Plus TYPES commit path (`commit/task-types-commit.ts`) and per-section manual "+ add" entry (out-of-plan but brief-scoped). 268 wizard tests green, 0 src tsc errors, audit-design-system clean on the new UI.
> **6.14 Playwright E2E — full spec 6/6 GREEN 2026-06-14** (happy/resume/upload/dedupe/offline/template; agent-off skipped — no OPENAI key). The `dedupe` `test.fixme` is now a real **upload→merge** pair: seed a SKU-matching `products` row, upload a matching CSV → the card renders `data-state="merge"`/DUPLICATE + show-diff and BUILD IT's commit body carries `state:"merge"`+`matchedExistingId`=the live id (an UPSERT, not a 2nd create). Harness (`catalog-setup-auth.ts`) gained an `existingProducts` fixture + a `products` read branch, and — critically — a `method==="HEAD"` count branch returning `Content-Range: */1` (+ `Access-Control-Expose-Headers: content-range`, since Content-Range is CORS-protected cross-origin) so `useBaselineSeeded` reports a seeded baseline; without this the prereq-gate hid the shell and EVERY wizard E2E failed.
> **Not yet built:** 6.5 standalone required-field validation engine (the shell's `selectBlockers` covers the live price/name case), 6.6 dedupe extension (the existing `dedupe-matcher` covers it), 6.9 dedicated completion service tests (completion logic shipped in the commit route + `completion-notification`), 6.10 prerequisite-gate + offline-banner COMPONENT pair — **BOTH NOW SHIPPED** (offline-banner earlier; prerequisite-gate `GatePanel`/`PrerequisiteGate` built + wired into the route 2026-06-14, commits `cea65dcb`/`fa8edbbc`, blocker derived from live company/baseline/subscription state via `useBaselineSeeded`, 10 component tests, audit clean), 6.12/6.13 coverage sweep + canvas/card component tests (canvas/cards have behavioral tests; a formal coverage floor pass is pending), 6.15 final done-gate sweep.
...
### Wave 3 — the three open red-team items finished (2026-06-15)

**1. xlsx CVE migration (P1-3) DONE.** Migrated off the deprecated `xlsx@0.18.5` npm build (prototype-pollution/ReDoS, no npm fix) to the maintained `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz` pin. package.json + the branch package-lock.json reconciled (`npm install --package-lock-only` — integrity hash present, the 8 orphan SheetJS transitive deps cfb/ssf/wmf/word/crc-32/adler-32/codepage/frac PRUNED → `npm ci`/Vercel merge-clean). 0.20.3's ESM build exports `read`/`utils` as named exports (verified) so `parseXlsx` is unchanged; round-trip-verified against the maintained build. **node_modules CAVEAT:** the shared symlinked node_modules locally still physically holds 0.18.5 — it can't be safely swapped mid-parallel-session (the auto-mode classifier correctly blocks writing the shared tree), so local runs resolve 0.18.5 (API-identical); the lockfile is the source of truth and a real deploy resolves 0.20.3.

**2. Per-field show-diff accept UI (P1-13) BUILT.** A merge (duplicate) SELL card renders a verdict toggle per changed field — TAKE INCOMING (olive, overwrite) vs KEEP ON FILE (the live value stands) — defaulting to take-all (doing nothing = the prior whole-card merge). Verdict lives on `StagingCard.fieldSelections` (`SET_FIELD_SELECTION`); the commit adapter rebuilds the merge doc FROM the on-file row and overrides only accepted fields. Only the RPC-committable fields get a toggle — **name / base_price / is_taxable**; `unit_cost` excluded (the RPC never writes products.unit_cost). Bulk shortcuts unified to the toggle vocabulary: KEEP ON FILE (every verdict → keep, non-destructive — the card stays) / TAKE INCOMING (re-bind + clear verdicts); the card's × REJECT is the only drop. `applyDedupe`/`CardResolution.fieldSelections` kept as the matcher-lane primitive (comments corrected). Design-skill stack invoked (frontend-design + interface-design + ops-copywriter + audit-design-system CLEAN); tokens-only, olive=verdict (never accent), neutral `ring-white/40` focus ring, EASE_SMOOTH.

**3. Live runtime walkthrough DONE.** Per-field UI verified live on `/catalog-setup-preview` (default take-all + mixed verdict + non-destructive KEEP ON FILE, screenshots captured); the real `/catalog/setup` route mounts live for an authed Maverick operator (permission + prerequisite + session-lock gates all pass, clean source-picker entry). The 0/0 first-run TAKEOVER banner could not be triggered live (all dev-bypass demo users are on the populated Maverick company); it remains E2E-covered (happy-path seeds 0/0 → wizard → BUILD IT → completed).

**Adversarial review (6-lens workflow) caught + fixed a PRE-EXISTING HIGH data-integrity bug the per-field contract exposed:** a `merge` previously sent the full incoming doc, so `catalog_setup_save`'s set-all-columns UPSERT WIPED non-diffed columns (description, category_id, show_in_storefront) and FORCE-REACTIVATED retired products (`is_active:true`) — contradicting the `[ the rest stays on file ]` copy. FIXED: a merge doc is rebuilt from the on-file row (full committable column set threaded via `existing-rows.OnFileProduct` + the products SELECT + a client→body→route hand-off), overriding only accepted verdicts; `is_active` carried from on-file (never reactivates); `show_in_storefront` added to the builder so it survives; an EDITED matched card keeps its upsert binding (`targetId` honors `matchedExistingId` regardless of state — was a duplicate-creation bug); a stale merge whose live row vanished falls back to create (never resurrects by id). Also: the COST/MARGIN data row shows the on-file cost on a merge (the RPC never changes cost); per-field toggle no-op same-ref; WCAG focus rings; orphaned `UNRESOLVE_CARD` deleted + the lying selectors comment corrected.

### Wave 3b — the two flagged gaps FIXED (2026-06-16, Jackson: "those need to be fixed")

**unit_cost persistence (P1-4/10) FIXED — `88837f5f`.** Rather than re-creating the 6386-line iOS-shared RPC, used the team's established pattern for "the RPC ignores column X" (the same one external_id uses): a post-commit stamp. `cost-stamp.ts` (`collectCostStampTargets` + `stampUnitCost`) writes each CREATED card's cost onto its row (id resolved from the RPC id_map), awaited but non-fatal (a miss leaves the pre-existing NULL, surfaced as `commitCostWarning`). MERGE cards are excluded — the RPC leaves a matched row's unit_cost untouched, so the on-file cost already survives, and cost is not a per-field verdict. 12 new tests (unit + 2 route). The buildDiff/data-row comments are corrected (cost is no longer an unfixed gap). NOTE: this fixes the WEB wizard; if iOS guided-setup shares `catalog_setup_save` it has the same RPC-level gap — the systemic RPC change (write unit_cost in the INSERT + both UPDATE-SETs, iOS-safe nullable) would fix both atomically and remains a reasonable follow-up.

**package-lock.json registry-less (P1-14) FIXED — `65445fa5`.** Diagnosed as PRE-EXISTING REPO-WIDE (979/1394 bare nodes on this branch AND on the primary `feat/inbox-dark-launch` checkout — the whole repo's lock was generated without registry metadata). Regenerating drifts 386 deps within their `^` ranges (an `npm update` in disguise — verified in /tmp) → unsafe + would break the shared node_modules for parallel sessions. Instead, surgically backfilled `resolved`+`integrity` for each node's EXACT locked version straight from the registry: ZERO version drift, node_modules untouched (still a symlink), canonical key order. Validated with `npm ci --dry-run` (1394 packages resolve, exit 0). The same backfill should eventually land on main/inbox (it's repo-wide), but the wizard branch is now `npm ci`-clean.

**STILL FLAGGED (out of scope — needs Jackson / a prod data op):**
- firebase_uid backfill for unprovisioned active operators (prod DATA write, app-wide — not a wizard change).
- (optional) the systemic `catalog_setup_save` unit_cost RPC change for iOS parity (the web wizard is fixed via the stamp).

Final verify: 519 unit (53 files), 0 src tsc errors, E2E 7/7, audit-design-system CLEAN, lockfile npm-ci-clean (zero drift). LOCAL only, no push.
