...
**STILL FLAGGED for Jackson (out of scope / pre-existing, NOT fixed here):**
- **unit_cost never persisted (P1-4/10):** `catalog_setup_save` has no `unit_cost` column in its products INSERT/UPDATE — wizard SELL commits drop cost on create AND merge (13/22 prod rows carry a positive cost the wizard can't set/preserve). Needs an additive RPC change (write `unit_cost` in the INSERT + both UPDATE-SET clauses; iOS-safe nullable). The per-field diff + the merge data row are honest about this (cost excluded / on-file shown), but a wizard-CREATED product still lands with NULL cost.
- **package-lock.json registry-less (P1-14):** ~979/1393 lock nodes lack `resolved`+`integrity` (pre-existing, whole-branch) — the xlsx node is clean but a full `npm ci` reproducibility pass wants the lock regenerated against the public registry. Separate from this diff.
- firebase_uid backfill for unprovisioned active operators (prod DATA write, app-wide).

Final verify: 507 unit (52 files across `src/lib/catalog-setup` + `src/components/catalog-setup` + `src/app/api/catalog/setup` + `tests/unit/catalog-setup`), 0 src tsc errors, E2E 7/7 (incl. the new on-file-preservation assertion), audit-design-system CLEAN, tree = my 27 files only (no sibling WIP). LOCAL only, no push.
