...
> **Remediated 2026-06-13 (P4-1-5) — with a finding correction.** Recommendation (a) shipped: `--fin-current` = olive `#9DB582` added system-wide, Dashboard receivables widget repointed at the canonical Books ramp (live-verified current = `rgb(157,181,130)` olive, identical to Books AGING). **Correction to this section:** the Dashboard "current" bucket was **not** an off-palette `#417394` literal — it was `WT.accent` = `var(--color-ops-accent)`, i.e. the **runtime accent**, which `PreferencesApplier` overrides per-user from the curated accent palette (the demo users' accent default *is* `#417394`; the static `globals.css` default is `#6F94B0`). So this was an **accent leak onto a data bar** (DESIGN.md §3), worse than a stray literal — a user with any other accent would get that color on the bucket. The "verified the accent does not leak" note was a methodology gap (the scan didn't follow `WT.accent`). On (c): an exhaustive classification of all 74 `#417394` occurrences found **zero** financial/aging and **zero** internal status-dot uses — every one is a legitimate portal white-label default, a user color-picker palette entry, or decorative chrome — so there was nothing to retire from the financial/status palette; the real fix was pinning the bucket to the static olive token. See decision log 2026-06-13 P4-1-5.
