...
The last two in-app client-create entry points still hopped through the `/clients/new` redirect — a double-hop (`push → null page → router.replace → param parse`) where every other caller already goes direct. Both repointed onto `useWindowStore.openClientWindow({ clientId: null, mode: "creating" })`:

- **⌘⇧C** (`keyboard-shortcuts.tsx`): `router.push("/clients/new")` → direct `getState().openClientWindow(...)` in the bare event handler (not a subscriber, so `getState()` not the hook). `useRouter` + the `[router]` effect dep stay — number-key nav and ⌘⇧P still use them.
- **Client-list widget "+ New client"** (`client-list-widget.tsx`): `navigate("/clients/new")` → a new `openClientWindow` selector, same idiom as the widget's existing `useWidgetEntityOpen` client path. `navigate` stays (the row actions still route to `/clients`, `/projects/new`, `/books`, `/tasks/new`).

**Route + layout handler untouched — deliberately.** `/clients/new` stays a byte-identical 15-line hand-off (`router.replace("/dashboard?openClient=new")`), and `dashboard-layout.tsx`'s `ClientWorkspaceDeepLinkHandler` still parses `openClient` and strips it. The `?openClient=new` deep link is a **contract**, not dead weight: agent action-cards build `/dashboard?openClient=<id>` URLs the handler must parse regardless. `git grep` confirmed **no** email/notification/drip link targets `/clients/new`, so unlike `/projects/new` it carries no hard external contract — the route survives purely as a cheap compatibility redirect for bookmarks/muscle memory.

**The idiom grammar unifies; the mechanisms deliberately do not flip.** Routes = permanent thin hand-offs. In-app callers = direct `useWindowStore` calls. Deep-link query params (`openProject=` / `openClient=`) are parsed once, in the layout. `/clients/new` KEEPS its query-param mechanism (it reuses the layout handler that must exist for action-cards anyway); `/projects/new` KEEPS its imperative mechanism (it must carry a `?clientId=` seed the layout handler doesn't parse). Both are locally optimal — neither flips.

**Setup gate NOT applied** (`openClientWindow` called directly, not via `dispatchQuickAction`): the catalog/setup gate is a Create-menu caller concern per the `quick-actions/dispatch.ts` docstring, and these two entry points never gated before. **`/tasks/new`** (the widget's Create-Task row action) stays untouched — separately filed follow-up, different contract.

**Store untouched.** `openClientWindow` already derives the `client-workspace:new` singleton id and, when the window exists, un-minimizes + raises z + replaces meta — so repeat opens refocus, never duplicate. A new unit test (`keyboard-shortcuts.test.tsx`) pins this against the REAL store: shortcut → one `client-workspace:new` window (`meta.initialMode "creating"`, `router.push` NOT called), second press → still one window with raised z, and ⌘⇧C-while-input-focused → zero windows (editable guard).

**Gates:** `tsc` — 7 pre-existing only (xlsx + notification-service), zero in touched files, zero new; `eslint` — **0 errors** on the 3 touched/created files (1 pre-existing `useScrollFadeScroll` unused-var warning on `client-list-widget.tsx:16`, identical on `origin/main`); `vitest` — new suite 3/3, targeted set 36/36, +`route-registry` 76/76 across 5 suites (registry undisturbed).

**Live-verified** (dev-bypass as PETE, 1600×900, Turbopack dev). The plan's "`.env.local` already wired" assumption did not hold — the file is absent from the worktree and even the main checkout lacked the two bypass flags; added locally for the run, `.env.local` + `.claude/launch.json` removed after (opening a create window mutates nothing server-side): **(a)** ⌘⇧C → NEW CLIENT window in place, URL stayed `/dashboard` (no route change); **(b)** second ⌘⇧C → one window, same position (singleton refocus); **(c)** widget "+ New client" → window opens, URL unchanged, second widget's + → still one window; **(d)** ⌘⇧C from a focused input → zero windows; **(e)** cold-load `/clients/new` → redirect to `/dashboard`, `openClient=new` stripped (`search === ""`), create window open — the untouched route + handler still honor the contract (network: `GET /dashboard?openClient=new → 200` then `GET /dashboard → 200`); **(f)** zero console errors (the "failed" requests are all sandbox-blocked external hosts — Typekit, GA/ads, Carto tiles — plus benign RSC-prefetch aborts, none from this change).

**Commits (`chore/clients-create-entry-consistency` off `origin/main` @ `2468d8b6`, Task 0 → World A):** `0622f3f2` (repoints + test), docs commit (this entry). **NOT pushed** — merging `ops-web` main auto-deploys prod.

*Merge-order note (World A): this file both-adds at EOF with `feat/projects-new-consolidation` (follow-up #1) and `claude/wizardly-shannon-dc59fe` — resolve keep-both in date order (#1 2026-07-03 before #2 2026-07-04). Plus the `keyboard-shortcuts.tsx` / `client-list-widget.tsx` resolutions per this pass's plan appendix (shared `useWindowStore` import kept once; `case "p"` = sibling's `openProjectWindow`, `case "c"` = ours; both widget selectors kept).*
