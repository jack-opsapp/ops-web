...
| `LEAD_SUMMARY_REFRESH_ENABLED` | Master switch for the recurring lead AI-summary refresh cron (`GET /api/cron/lead-summary-refresh`, hourly at :40 in the email-sync window). Unset or any value other than `"true"` → cron no-ops after auth with `{ skipped: true }` (the default; recurring LLM spend requires explicit opt-in per the cost gate). The one-time backfill (`POST` on the same path with `{"mode":"backfill"}` + CRON_SECRET) is deliberately NOT gated by this switch. Per-company `phase_c` gating applies in both modes. |
