-- Actor-authorized, prompt-bounded source snapshot for the shared OPS agent
-- domain service. Browser roles never receive direct execution authority.
...
begin
  foreach v_signature in array array[
    ...
    if to_regprocedure(v_signature) is null then
      raise exception
        'agent_job_conversation_context_prerequisite_missing: %',
        v_signature;
    end if;
  end loop;
  ...
  foreach v_table in array array[
    ...
    'public.job_memory_version_evidence',
...
comment on function public.read_agent_job_conversation_context_as_system(
  text, uuid, uuid, text, text[], text, text, text, text[], text, text,
  text, text, text, uuid, integer, text[], uuid
) is
  'Returns one current, actor-authorized job memory snapshot with latest exact turns, source evidence, participant provenance, minimal visible cross-job continuity, and current redactions evaluated in one source statement.';
