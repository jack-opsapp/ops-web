-- Forward repair of 20260830100000 (bug 4da75e71): agent_writing_profiles.user_id
-- is TEXT (uuid-as-text), but the merge function compared it bare against the
-- uuid parameter -- `operator does not exist: text = uuid` on every call, so no
-- subject was ever learnable. Caught by the backfill script's dry run before any
-- runtime caller burned on it (the runtime learner is fail-open by design).
-- Single change vs the original: `p.user_id = p_user_id` -> `p_user_id::text`.
