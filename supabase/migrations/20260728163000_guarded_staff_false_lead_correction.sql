...
-- Durable receipt for a reviewed staff-authored false-lead correction. The
-- service role may inspect receipts, but only the guarded function below can
-- write them.
create table public.lead_intake_correction_runs (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id) on delete cascade,
  correction_key text not null
    check (
      correction_key = btrim(correction_key)
      and length(correction_key) between 8 and 200
    ),
  actor_user_id uuid not null,
  source_opportunity_id uuid not null,
  manifest_sha256 text not null
    check (manifest_sha256 ~ '^[0-9a-f]{64}$'),
  entry_a_sha256 text not null
    check (entry_a_sha256 ~ '^[0-9a-f]{64}$'),
  entry_b_sha256 text not null
    check (entry_b_sha256 ~ '^[0-9a-f]{64}$'),
  input_spec jsonb not null check (jsonb_typeof(input_spec) = 'object'),
  result jsonb not null check (jsonb_typeof(result) = 'object'),
  applied_at timestamptz not null default clock_timestamp(),
  constraint lead_intake_correction_runs_actor_company_fkey
    foreign key (company_id, actor_user_id)
    references public.users (company_id, id)
    on delete restrict,
  constraint lead_intake_correction_runs_source_company_fkey
    foreign key (company_id, source_opportunity_id)
    references public.opportunities (company_id, id)
    on delete restrict,
  unique (company_id, correction_key)
);

create index lead_intake_correction_runs_source_idx
  on public.lead_intake_correction_runs (
    company_id,
    source_opportunity_id,
    applied_at desc
  );
create index lead_intake_correction_runs_actor_idx
  on public.lead_intake_correction_runs (
    company_id,
    actor_user_id,
    applied_at desc
...
revoke all on table public.lead_intake_correction_runs
  from public, anon, authenticated, service_role;
grant select on table public.lead_intake_correction_runs to service_role;

comment on table public.lead_intake_correction_runs is
  'Immutable, content-addressed receipts for service-only lead-intake data corrections. No role has direct write privileges.';

-- Rebuild only correspondence-derived opportunity fields. The existing
-- transaction capability suppresses the legacy updated_at touch when (and only
-- when) this exact projection-only update executes.
create or replace function private.recompute_staff_false_lead_projection(
  p_company_id uuid,
...
begin
  if p_company_id is null or p_opportunity_id is null then
    raise exception 'invalid_staff_false_lead_projection_request'
      using errcode = '22023';
  end if;

  select opportunity.updated_at
    into v_updated_at
  ...
  select
    count(*)::integer,
    count(*) filter (where event.direction = 'inbound')::integer,
    count(*) filter (where event.direction = 'outbound')::integer,
  ...
  on conflict (transaction_id, backend_pid, company_id, opportunity_id)
  do update set expected_updated_at = excluded.expected_updated_at;

  update public.opportunities opportunity
     set correspondence_count = v_correspondence_count,
   ...
   where opportunity.id = p_opportunity_id
     and opportunity.company_id = p_company_id
     and opportunity.deleted_at is null
  returning opportunity.updated_at into v_after_updated_at;
  if not found then
    raise exception 'opportunity_not_found' using errcode = 'P0002';
  end if;
  if v_after_updated_at is distinct from v_updated_at then
    raise exception 'projection_timestamp_changed' using errcode = '23514';
  end if;
end;
$function$;

revoke all on function private.recompute_staff_false_lead_projection(
  uuid, uuid
) from public, anon, authenticated, service_role;

-- Correct a proven staff-authored false lead without trusting names, domains,
-- locality strings, or fuzzy identity. All generated row ids are supplied by
-- the database at execution; the migration contains no production record ids.
...
begin
  ...
  loop
    ...
    if v_reference_count <> 0 then
      raise exception 'client_reference_remains: %.%',
        v_reference.table_name,
        v_reference.column_name
        using errcode = '23514';
    end if;
  ...
  if exists (
    ...
    raise exception 'client_reference_remains: clients.merged_into_client_id'
      using errcode = '23514';
  end if;
