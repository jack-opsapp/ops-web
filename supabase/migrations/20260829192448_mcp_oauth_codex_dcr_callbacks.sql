-- Add strict native Codex DCR callbacks without weakening redirect binding.
-- Codex resolves its ephemeral loopback port before registration, so the
-- registered URI remains byte-exact through consent, code, and token exchange.
...
begin
  ...
  if v_codex_redirect_count > 0
     and (
       v_codex_redirect_count <> cardinality(p_redirect_uris)
