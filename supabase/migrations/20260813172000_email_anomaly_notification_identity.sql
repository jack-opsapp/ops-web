...
begin
  ...
  if p_anomaly_id is null
     ...
     or pg_catalog.nullif(pg_catalog.btrim(p_title), '') is null
     or pg_catalog.nullif(pg_catalog.btrim(p_body), '') is null then
  ...
  if not exists (
    ...
    where recipient.id = p_user_id
      ...
      and pg_catalog.coalesce(recipient.is_active, false)
  ) then
  ...
  insert into public.notifications as notification (
    ...
    false,
    p_persistent,
    pg_catalog.nullif(pg_catalog.btrim(p_action_url), ''),
    pg_catalog.nullif(pg_catalog.btrim(p_action_label), ''),
    v_dedupe_key
  )
...
begin
  ...
  select distinct
    ...
    'Reason: ' || pg_catalog.coalesce(v_audit.reason, 'Automatic safety pause') ||
      '. Paused by ' || pg_catalog.coalesce(v_audit.actor_email, 'OPS automation') || '.',
    false,
    true,
