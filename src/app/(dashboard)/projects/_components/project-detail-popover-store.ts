...
// ── Types ──
export type ProjectPopoverTab = "overview" | "tasks" | "financial" | "photos";

export interface ProjectDetailPopoverState {
  id: string;
  title: string;
  statusColor: string;
  ...
  zIndex: number;
  isMinimized: boolean;
  activeTab: ProjectPopoverTab;
}

interface ProjectDetailPopoverStoreState {
  popovers: Map<string, ProjectDetailPopoverState>;
  nextZIndex: number;

  openPopover: (projectId: string, cardScreenPosition: { x: number; y: number }, title: string, statusColor: string) => void;
  ...
  setActiveTab: (id: string, tab: ProjectPopoverTab) => void;
}
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, ProjectDetailPopoverState>
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useProjectDetailPopoverStore = create<ProjectDetailPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (projectId, cardScreenPosition, title, statusColor) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(projectId);
