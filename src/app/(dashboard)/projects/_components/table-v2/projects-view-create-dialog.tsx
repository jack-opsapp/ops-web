...
import { createDefaultProjectViewDefinitionInput } from "@/lib/utils/project-view-defaults";
...
const PROJECT_VIEW_NAME_MAX_LENGTH = 60;

type ProjectViewActions = ReturnType<typeof useProjectViewActions>;
...
function getMutationErrorCode(error: unknown): ProjectTableViewMutationErrorCode {
...
function viewErrorCopyKey(code: ProjectTableViewMutationErrorCode) {
  switch (code) {
    case "DUPLICATE_NAME":
...
function buildDuplicateName(template: string, viewName: string) {
  ...
  if (generated.length <= PROJECT_VIEW_NAME_MAX_LENGTH) return generated;
  return generated.slice(0, PROJECT_VIEW_NAME_MAX_LENGTH).trim();
}

export function ProjectsViewCreateDialog({
  open,
  mode,
  activeView,
  actions,
  ...
  open: boolean;
  mode: CreateMode;
  activeView: ProjectTableViewDefinition | null;
  actions: ProjectViewActions;
  onOpenChange: (open: boolean) => void;
  onViewCreated: (view: ProjectTableViewDefinition) => void;
}) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (trimmedName.length > PROJECT_VIEW_NAME_MAX_LENGTH) {
    ...
    try {
      const createdView = await actions.createPersonalView.mutateAsync(
        source === "current" && activeView
          ...
          : { name: trimmedName, definition: createDefaultProjectViewDefinitionInput() },
      );
      onViewCreated(createdView);
      onOpenChange(false);
    } catch (error) {
      ...
      <DialogContent className="glass-dense max-w-[420px] rounded-modal p-0" hideClose>
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <DialogFooter className="border-t border-border px-3 py-2">
            <DialogClose
              type="button"
              className="rounded px-3 py-1.5 font-cakemono text-[14px] font-light uppercase text-text-3 transition-colors hover:bg-surface-hover hover:text-text focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ops-accent"
            >
