...
const CARD_OFFSET_X = 220; // CARD_WIDTH (200) + 20px gap
const CASCADE_OFFSET = 30;

// ── Types ──
export type PopoverTab = "correspondence" | "timeline" | "photos";

export interface DetailPopoverState {
  id: string; // same as opportunityId
  title: string;
  stageColor: string; // for dock pill display
...
interface DetailPopoverStoreState {
  popovers: Map<string, DetailPopoverState>;
  nextZIndex: number;

  openPopover: (opportunityId: string, cardScreenPosition: { x: number; y: number }, title: string, stageColor: string) => void;
  ...
  setActiveTab: (id: string, tab: PopoverTab) => void;
}
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, DetailPopoverState>
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useDetailPopoverStore = create<DetailPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (opportunityId, cardScreenPosition, title, stageColor) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(opportunityId);

      if (existing) {
        // Already open — restore if minimized, otherwise just focus
        const updated = new Map(popovers);
        updated.set(opportunityId, {
          ...existing,
          isMinimized: false,
          zIndex: nextZIndex,
        });
      ...
      const baseX = cardScreenPosition.x + CARD_OFFSET_X;
      const baseY = cardScreenPosition.y;
