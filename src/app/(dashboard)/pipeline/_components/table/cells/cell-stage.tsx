...
export function CellStage({ stage }: { stage: OpportunityStage }) {
  const color = OPPORTUNITY_STAGE_COLORS[stage] ?? "#8A8A8A";
  return (
    <span className="inline-flex min-w-0 max-w-full items-center gap-[6px] rounded-chip font-mono text-micro uppercase tracking-wider text-text-2">
      <span
        aria-hidden="true"
