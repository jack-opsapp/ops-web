...
function Card({ card }: { card: KanbanCard }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-[6px] border border-white/[0.08] bg-[rgba(18,18,20,0.58)] p-2.5 backdrop-blur-[20px] transition-colors duration-150 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-white/[0.20] hover:bg-white/[0.04]"
    >
      ...
      {(holdLabel || card.nextActionLabel || card.isTest) && (
        <div className="mt-1.5 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.12em]">
          {holdLabel && (
            <span className="rounded-[4px] border border-rose/30 px-1.5 py-px text-rose">
              {holdLabel}
            </span>
          )}
          {card.isTest && (
            <span className="rounded-[4px] border border-tan/40 px-1.5 py-px text-tan">
              TEST
            </span>
          )}
