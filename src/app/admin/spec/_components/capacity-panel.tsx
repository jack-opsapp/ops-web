    ...
    >
      <div className="mb-4 flex items-baseline justify-between">
        ...
        <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-text-mute">
          <span className="text-text-mute">[</span>READ ONLY · EDIT VIA /ADMIN/SPEC/CAPACITY
  ...
  return (
    <div className="glass-surface p-5">
      ...
      <div className="mt-3 flex items-baseline gap-2">
        <span className="font-mono text-[26px] tabular-nums text-text leading-none">
          {row.active}
        </span>
        <span className="font-mono text-[14px] tabular-nums text-text-3 leading-none">
          / {row.slotCeiling}
        </span>
        <span className="ml-auto font-mono text-[11px] uppercase tracking-[0.12em] text-text-mute">
          {ratioPct}% UTIL
        </span>
      </div>
      ...
      <dl className="mt-4 grid grid-cols-3 gap-x-4 font-mono text-[11px] uppercase tracking-[0.12em] text-text-3">
        <div>
          <dt className="text-text-mute">QUEUE</dt>
          <dd className="mt-1 tabular-nums text-text">{row.queued}</dd>
        </div>
        <div>
          <dt className="text-text-mute">HOLD · CUST</dt>
          <dd className="mt-1 tabular-nums text-text">{row.holdCustomerRequested}</dd>
        </div>
        <div>
          <dt className="text-text-mute">HOLD · OPS</dt>
          <dd className="mt-1 tabular-nums text-text">{row.holdOpsBlocked}</dd>
        </div>
      </dl>
      ...
      <div className="mt-4 space-y-1.5">
        <div className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em]">
          <span className="text-text-mute">NEXT START</span>
        ...
        {row.publicNote ? (
          <p className="text-[11px] text-text-2">
            <span className="font-mono text-text-mute">NOTE</span> · {row.publicNote}
          </p>
        ) : null}
      </div>
    </div>
