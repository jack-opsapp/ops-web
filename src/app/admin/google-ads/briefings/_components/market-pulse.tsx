          ...
          <div key={i} className="border border-white/[0.08] rounded p-4 bg-white/[0.02]">
            ...
            {theme.quotes.length > 0 && (
              <div className="space-y-1 mb-2">
                {theme.quotes.map((quote, qi) => (
                  <p key={qi} className="font-mono text-[12px] text-[#A0A0A0] italic border-l border-white/[0.06] pl-3">
                    &ldquo;{quote}&rdquo;
                  </p>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2">
              {theme.sources.map((src, si) => (
                <span key={si} className="font-mono text-micro text-[#444444] bg-white/[0.04] px-1.5 py-0.5 rounded">
                  {src}
                </span>
              ))}
            </div>
