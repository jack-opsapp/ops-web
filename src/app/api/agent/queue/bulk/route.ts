    ...
    try {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "actionIds (non-empty array) and action ('approve'|'reject') are required",
          },
          { status: 400 }
        );
      }
      ...
      let result;
      if (action === "approve") {
        result = await ApprovalQueueService.bulkApprove(
          actionIds,
          auth.companyId,
          auth.id
        );
      } else {
        result = await ApprovalQueueService.bulkReject(
          actionIds,
          auth.companyId,
          auth.id,
          notes
