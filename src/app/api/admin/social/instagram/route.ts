...
const defaults: AdminSocialInstagramRouteDependencies = {
  ...
  service: {
    getPublicStatus: () =>
      createInstagramConnectionService().getPublicStatus(),
  ...
  return {
    GET: async (request: NextRequest): Promise<NextResponse> => {
      try {
        ...
        return json({ connection: await dependencies.service.getPublicStatus() });
      } catch (error) {
    ...
    POST: async (request: NextRequest): Promise<NextResponse> => {
      try {
        ...
        const authorizationUrl = await dependencies.service.createAuthorizationUrl(
          user.email!
        );
        ...
        return json({ error: "Instagram connection could not be started" }, 500);
      }
    },
