...
interface SocialPublishCronDependencies {
  runBatch: (options: { limit: number }) => ReturnType<typeof runSocialPublisherBatch>;
}
...
export function createSocialPublishCronHandler(
  ...
  return async function handleSocialPublishCron(request: NextRequest): Promise<NextResponse> {
    const cronSecret = process.env.CRON_SECRET?.trim() ?? "";
    if (cronSecret.length < 32) {
      return NextResponse.json(
        { ok: false, code: "CRON_AUTH_NOT_CONFIGURED", error: "Cron authentication is not configured" },
        { status: 503 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, code: "SOCIAL_PUBLISH_WORKER_FAILED", error: "Social publish worker failed" },
        { status: 500 }
      );
    }
  };
