...
export function ImportPipelineWizard({
  ...
  const [provider, setProvider] = useState<"gmail" | "microsoft365" | null>(
    null
  );
  ...
  const saveReviewState = useCallback(async () => {
    ...
    const reviewState = {
      ...
      filteredOutIds: confirmedLeads
        .filter((l) => !l.enabled && l.needsReview)
        .map((l) => l.id),
      ...
      triageDecisions: Array.from(triageDecisions.entries()).map(
        ([leadId, decision]) => ({ leadId, decision })
      ),
    ...
    try {
      await authedFetch("/api/integrations/email/connection", {
        ...
        body: JSON.stringify({
          connectionId,
          syncFilters: {
            reviewState: { ...reviewState, savedAt: new Date().toISOString() },
          },
        }),
      });
  ...
  // ─── State restoration ──────────────────────────────────────────────────────
  const [stateCheckComplete, setStateCheckComplete] =
          ...
          try {
            if (filters.lastImportJobId) {
              const jobRes = await authedFetch(
                `/api/integrations/email/analyze-status?jobId=${filters.lastImportJobId}`
              );
              if (jobRes.ok) {
            ...
            if (filters.lastScanJobId) {
              const scanRes = await authedFetch(
                `/api/integrations/email/analyze-status?jobId=${filters.lastScanJobId}`
              );
              if (scanRes.ok) {
                ...
                if (scanData.result) {
                  ...
                  if (scanData.result.estimatePattern)
                    setEstimatePattern(scanData.result.estimatePattern);
                }
              }
            }
          } catch (err) {
            console.error(
              "[wizard] Failed to load existing scan/import data for active connection:",
              err
            );
          }
            ...
            if (jobData.status === "import_complete" && jobData.result) {
              ...
              if (jobData.progress) {
                setBgProgress({
                  ...
                  message: jobData.progress.message || t("wizard.importing"),
                });
              }
        ...
        if (filters.lastScanJobId) {
          ...
          if (jobData.status === "complete" && jobData.result) {
            ...
            if (reviewState?.savedAt) {
              const hoursSince =
                (Date.now() - new Date(reviewState.savedAt).getTime()) /
                (1000 * 60 * 60);
              if (hoursSince < 24) {
                // Restore filter decisions
                const filteredSet = new Set(reviewState.filteredOutIds || []);
                const restoredLeads = (jobData.result.leads || []).map(
                  (l: AnalyzedLead) => ({
                    ...l,
                ...
                if (reviewState.nameOverrides?.length) {
                  const nameMap = new Map(
                    reviewState.nameOverrides.map(
                      (n: { leadId: string; name: string }) => [
                        n.leadId,
                        n.name,
                      ]
                    )
                  ...
                  for (const lead of restoredLeads) {
                    ...
                    if (nameOverride)
                      lead.client = { ...lead.client, name: nameOverride };
                  }
                }
                ...
                if (reviewState.stageOverrides?.length) {
                  const stageMap = new Map(
                    reviewState.stageOverrides.map(
                      (s: { leadId: string; stage: string }) => [
                        s.leadId,
                        s.stage,
                      ]
                    )
                ...
                if (reviewState.triageDecisions?.length) {
                  ...
                  for (const {
                    leadId,
                    decision,
                  } of reviewState.triageDecisions) {
                ...
                const enabledLeads = restoredLeads.filter(
                  (l: AnalyzedLead) => l.enabled
                );
                const groups = buildConsolidationGroups(enabledLeads);
                if (reviewState.consolidationDecisions?.length) {
                  const decisionMap = new Map(
                    reviewState.consolidationDecisions.map(
                      (d: {
                        groupId: string;
                        decision: string;
                        companyName?: string;
                  ...
                  for (const group of groups) {
                    const saved = decisionMap.get(group.id) as
                      | {
                          groupId: string;
                          decision: string;
                          companyName?: string;
                    ...
                    if (saved) {
                      ...
                      if (saved.companyName)
                        group.companyName = saved.companyName;
                    }
                  }
                  ...
                  for (const group of groups) {
                    if (group.decision === "merge" && group.leads.length > 1) {
                      const secondaryIds = new Set(
                        group.leads.slice(1).map((gl) => gl.leadId)
                      );
                      for (const lead of restoredLeads) {
                ...
                lastSaveFingerprintRef.current = JSON.stringify({
                  ...
                  triageDecisions: (reviewState.triageDecisions || []).map(
                    (t: { leadId: string; decision: TriageDecision }) => ({
                      leadId: t.leadId,
                      decision: t.decision,
                    })
                  ),
            ...
            if (jobData.progress) {
              setBgProgress({
                percent: jobData.progress.percent || 0,
                message: jobData.progress.message || t("wizard.analyzing"),
              });
            }
            setDirection(1);
            setStep(2);
    ...
    const poll = async () => {
      try {
        ...
        const res = await authedFetch(
          `/api/integrations/email/analyze-status?jobId=${runningJobId}`
        );
        if (!res.ok) {
        ...
        if (data.progress) {
          setBgProgress({
            ...
            message: data.progress.message || t("wizard.working"),
          });
        ...
        if (
          data.status === "import_complete" &&
          data.result &&
          runningJobType === "import"
        ) {
          ...
          setBgProgress({
            percent: 100,
            message: t("importProgress.complete"),
          });
          invalidateConnections();
        ...
        if (data.status === "error" || data.status === "import_error") {
          setBgProgress({
            percent: 0,
            message: data.error || t("importProgress.failed"),
          });
          setRunningJobId(null);
          setRunningJobType(null);
          return;
    ...
    try {
            ...
            decision === "won"
              ...
              : decision === "lost"
                ...
                : decision === "discard"
                  ? "discarded"
                  : lead.stage;
          const messagesByDate = [...lead.emails]
            .filter((email) => Number.isFinite(Date.parse(email.date)))
            .sort(
              (left, right) => Date.parse(left.date) - Date.parse(right.date)
            );
          let lastInboundAt: string | null = null;
          let lastOutboundAt: string | null = null;
          for (const email of messagesByDate) {
            if (email.direction === "inbound") {
              lastInboundAt = email.date;
            } else {
              lastOutboundAt = email.date;
            }
          }
          const lastMessageDirection = messagesByDate.at(-1)?.direction ?? null;

          return {
            id: lead.id,
            threadId: lead.threadId,
            providerThreadId: lead.providerThreadId,
            ...
            action: lead.matchResult
              .action as ImportPayload["leads"][number]["action"],
            mergeMode: lead.mergeMode,
            mergeWithLeadId: lead.duplicateGroupId,
            subContacts: lead.subContacts || [],
            title: titleMap.get(lead.id) || null,
            actualCloseDate:
              stage === "won" || stage === "lost" || stage === "discarded"
                ? lead.lastMessageDate || null
                : null,
          };
        }),
        syncProfile: {
      ...
      if (!res.ok) {
        const errorData = await res
          .json()
          .catch(() => ({ error: t("wizard.importFailedDefault") }));
        throw new Error(
          errorData.error ||
            t("wizard.importFailedStatus", { status: res.status })
        );
      }
      ...
      setBgProgress({
        percent: 0,
        message: t("wizard.startingImport", { count: importLeads.length }),
      });
      invalidateConnections();
    } catch (err) {
      ...
      toast.error(
        err instanceof Error ? err.message : t("wizard.importFailed")
      );
    }
  }, [
    connectionId,
  ...
  const handleActivate = useCallback(
    async (
      ...
      if (!res.ok) {
        const errorData = await res
          .json()
          .catch(() => ({ error: t("wizard.activationFailedDefault") }));
        throw new Error(
          errorData.error ||
            t("wizard.activationFailedStatus", { status: res.status })
        );
      }
  ...
  const minimizedLabel = isImportPhase
    ? t("wizard.minimizedImporting")
    : t("wizard.minimizedAnalyzing");
  const minimizedCompleteLabel = isImportPhase
    ? t("wizard.importComplete", { count: importResult?.leadsCreated ?? 0 })
    : t("wizard.analysisComplete", {
        count: analysisResult?.leads?.length ?? 0,
      });
    ...
    <>
      ...
      {minimized && !open && (
        <motion.div
          ...
          className="glass-dense fixed bottom-4 right-4 z-[100] flex cursor-pointer items-center gap-3 rounded-chip border border-border px-4 py-3"
          style={{
            minWidth: 280,
          }}
          onClick={() => {
        ...
        >
          {bgComplete ? (
            <>
              ...
              <span className="shrink-0 font-mohave text-[12px] text-text">
                {t("wizard.review")}
              </span>
            </>
          ) : (
            <>
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <span className="ml-auto shrink-0 font-mohave text-[11px] text-text-3">
                ...
                {bgVisibleName && runningJobType === "analysis" && (
                  <AnimatePresence mode="wait">
                    <motion.p
                      ...
                      exit={{ opacity: 0 }}
                      transition={{ duration: 0.3, ease: EASE }}
                      className="mb-1 truncate font-mohave text-[11px] text-text-3"
                    >
              ...
              <span className="ml-2 shrink-0 font-mohave text-[12px] text-text-2 transition-colors hover:text-text">
                {t("wizard.expand")}
              </span>
            </>
          )}
      ...
      >
        <DialogContent
          className="w-[90vw] max-w-[920px] overflow-hidden rounded-chip border border-border bg-black p-0"
          hideClose
          onKeyDown={(e) => {
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border px-6 pb-4 pt-5">
            <div>
              ...
              <p className="font-mono text-micro uppercase tracking-[0.15em] text-text-3">
                {t(`steps.${STEP_KEY_MAP[step]}`)}
                {step === 4
                  ? ` · ${t(`subSteps.${SUB_STEP_KEY_MAP[reviewSubStep]}`)}`
                  : ""}
              </p>
            </div>
            <button
              ...
              className="p-1.5 text-text-3 transition-colors hover:text-text"
            >
              <X size={16} />
            </button>
          </div>
          ...
          >
            {/* Stepper rail */}
            <div className="flex-shrink-0 pb-4 pl-5 pt-4">
              <StepperRail
                steps={stepperSteps}
                currentStep={STEP_KEY_MAP[step]}
                currentSubStep={
                  step === 4 ? SUB_STEP_KEY_MAP[reviewSubStep] : undefined
                }
            ...
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden px-6 pb-2 pt-4">
              <AnimatePresence mode="wait" custom={direction}>
                ...
                >
                  {step === 1 && <ConnectStep companyId={companyId} />}
                  {step === 2 && !connectionId && (
                    <div className="flex flex-col items-center justify-center gap-4 py-16">
                      ...
                      <button
                        onClick={() => goTo(1)}
                        className="font-mono text-micro uppercase tracking-[0.1em] text-text-2 transition-colors hover:text-text"
                      >
                  ...
                  {step === 2 &&
                    ...
                    (!stateCheckComplete ? (
                      ...
                      <AnalyzeStep
                        ...
                        companyId={companyId}
                        existingJobId={
                          runningJobId || existingJobId || undefined
                        }
                      ...
                      onNext={() => {
                        ...
                        const allKnownSources = new Set(
                          confirmedSources.map(
                            (s) => sourceTypeToLeadSource[s.type] || s.type
                          )
                        );
                        ...
                        const filtered = (analysisResult.leads || []).map(
                          (lead) => ({
                            ...lead,
                            enabled: allKnownSources.has(lead.source)
                              ? enabledLeadSources.has(lead.source)
                                ? lead.enabled
                                : false
                              : lead.enabled,
                          })
                        ...
                        const hasFlagged = filtered.some(
                          (l) => l.needsReview && l.enabled
                        );
                        if (!hasFlagged) {
                          const groups = buildConsolidationGroups(
                            filtered.filter((l) => l.enabled)
                          );
                          setConsolidationGroups(groups);
                    ...
                    (importJobId ? (
                        ...
                        onBack={() => {
                          ...
                          for (const g of consolidationGroups) {
                            if (g.decision === "merge" && g.leads.length > 1) {
                              g.leads
                                .slice(1)
                                .forEach((gl) => mergedLeadIds.add(gl.leadId));
                            }
                          }
                          if (mergedLeadIds.size > 0) {
                            setConfirmedLeads((prev) =>
                              prev.map((l) =>
                                mergedLeadIds.has(l.id)
                                  ? { ...l, enabled: true }
                                  : l
                              )
                            );
                          ...
                          const hasFlagged = confirmedLeads.some(
                            (l) => l.needsReview && l.enabled
                          );
                          if (hasFlagged) {
                            setReviewSubStep(1);
                      ...
                      <ConfirmPipelineStep
                        ...
                        onStageChange={(id, stage) => {
                          ...
                          setTriageDecisions((prev) =>
                            new Map(prev).set(id, newDecision)
                          );
                        }}
                  ...
                  {step === 5 && importResult && (
                    <ActivateStep
                      ...
                      companyId={companyId}
                      syncProfile={{
                        estimateSubjectPatterns: estimatePattern
                          ? [estimatePattern]
                          : [],
                        companyDomains: analysisResult?.companyDomains || [],
                        teamForwarders: analysisResult?.teamForwarders || [],
                        ...
                        formSubjectPatterns: confirmedSources
                          .filter(
                            (s) => s.type === "estimate_pattern" && s.enabled
                          )
                          .map((s) => s.pattern),
                        userEmailAddresses:
                          analysisResult?.teamForwarders || [],
                        aiClassificationThreshold: 0.7,
                      }}
            ...
            {showCloseConfirm && (
              ...
              >
                <div className="glass-dense max-w-[320px] rounded-chip border border-border p-6">
                  <p className="mb-2 font-mohave text-[15px] text-text">
                    {t("wizard.closeTitle")}
                  </p>
                  <p className="mb-5 font-mohave text-[12px] text-text-3">
          ...
          {/* end flex layout (stepper rail + content) */}
          ...
          {step > 3 && !importJobId && !runningJobId && step !== 4 && (
            <div className="flex items-center justify-between px-6 pb-4">
              <button
                ...
                className="font-mohave text-[13px] text-text-3 transition-colors hover:text-text"
              >
                {t("wizard.back")}
              </button>
            </div>
