...
const normalizeStatus = (s: string) =>
  s === "building_leads" ? "analyzing_threads" : s;

const STAGES = [
  {
    key: "analyzing_sent",
    icon: Mail,
    labelKey: "analyze.stage.analyzing_sent",
    range: [5, 35],
  },
  {
    key: "detecting_platforms",
    icon: Search,
    labelKey: "analyze.stage.detecting_platforms",
    range: [35, 50],
  },
  {
    key: "classifying_ai",
    icon: Zap,
    labelKey: "analyze.stage.classifying_ai",
    range: [50, 70],
  },
  {
    key: "analyzing_threads",
    icon: MessageCircle,
    labelKey: "analyze.stage.analyzing_threads",
    range: [70, 95],
  },
  {
    key: "complete",
    icon: CheckCircle,
    labelKey: "analyze.stage.complete",
    range: [100, 100],
  },
];
...
export function AnalyzeStep({
  ...
  const [status, setStatus] = useState<string>(
    existingJobId ? "analyzing_sent" : "pending"
  );
  const [serverProgress, setServerProgress] = useState(0);
  const [displayProgress, setDisplayProgress] = useState(0);
  const [message, setMessage] = useState(
    existingJobId ? t("analyze.reconnecting") : t("analyze.starting")
  );
  ...
  useEffect(() => {
    ...
    const maxForStage = currentStage
      ? currentStage.range[1] - 1
      : displayProgress;
  ...
  useEffect(() => {
    ...
    const startAnalysis = async () => {
      try {
        const res = await authedFetch("/api/integrations/email/analyze", {
          method: "POST",
      ...
      try {
        ...
        if (data.progress) {
          ...
          const stageIndex = STAGES.findIndex(
            (s) => s.key === normalizeStatus(data.progress.stage)
          );
          if (stageIndex >= 0) {
  ...
  return (
    <div>
      <p className="mb-8 font-mohave text-[15px] text-text-2">
        {existingJobId ? t("analyze.reconnectingIntro") : t("analyze.intro")}
      </p>

      {error ? (
        <div className="rounded border border-brick-line bg-rose-soft p-4">
        ...
        <div className="space-y-4">
          {/* Progress bar */}
          <div>
            <div className="h-[2px] w-full overflow-hidden rounded-bar bg-white/5">
              <motion.div
                className="h-full"
                initial={{ width: "0%" }}
                animate={{
            ...
            <div className="mt-2 flex items-center justify-between">
              <p
                className="font-mono text-[12px] tabular-nums transition-colors duration-500"
                style={{ color: isComplete ? "var(--olive)" : "var(--text-3)" }}
              >
                {isComplete && (
                  <CheckCircle size={11} className="-mt-0.5 mr-1 inline" />
                )}
                {percentText}
              </p>
          ...
          <div className="space-y-2">
            {STAGES.map((stage, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex h-7 w-7 items-center justify-center rounded-chip border transition-all duration-300 ${
                      isStageCompleted
                  ...
                  >
                    {isStageCompleted ? (
                      ...
                      <div className="h-3 w-3 animate-spin rounded-full border-2 border-text-3/40 border-t-text" />
                    ) : (
                  ...
                  {isCurrent && (
                    <motion.span
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="ml-auto font-mohave text-[11px] text-text-3"
                    >
                      {message}
                    </motion.span>
                  )}
          ...
          {showMinimize && onMinimize && !isComplete && status !== "error" && (
            ...
            >
              <button
                onClick={onMinimize}
                className="flex items-center gap-2 rounded border border-border bg-white/5 px-4 py-2 font-mohave text-[13px] text-text-2 transition-all hover:border-border-medium hover:bg-surface-hover hover:text-text"
              >
                <Minimize2 size={14} />
                {t("analyze.minimize")}
              </button>
