        ...
        return (
          <div key={expense.id}>
            ...
            >
              ...
              {/* Category */}
              <span className="font-mono text-caption-sm text-text-3 truncate">
              ...
              <span>
                {isFlagged ? (
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-[rgba(196,168,104,0.15)] text-[#C4A868] font-mono text-micro uppercase tracking-wider">
                    <Flag className="w-[8px] h-[8px]" />
                    FLAGGED
                  </span>
                ) : expense.status === "approved" ? (
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-[rgba(157,181,130,0.15)] text-[#9DB582] font-mono text-micro uppercase tracking-wider">
                    APPROVED
                  </span>
                ) : expense.status === "rejected" ? (
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-[rgba(147,50,26,0.15)] text-[#93321A] font-mono text-micro uppercase tracking-wider">
                    REJECTED
                  </span>
                ) : null}
              </span>
                    ...
                    {isFlagged ? (
                      <div className="space-y-1.5">
                        <div className="flex items-start gap-2">
                          ...
                          <span className="font-mono text-caption-sm text-[#C4A868]">
                            {expense.flagComment}
                          </span>
                        </div>
                        <button
                          onClick={() => onUnflag(expense.id)}
                          className="flex items-center gap-1 font-mono text-micro text-text-3 hover:text-text-2 uppercase tracking-wider transition-colors"
                        >
                      ...
                      <div className="space-y-1.5">
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-[rgba(255,255,255,0.04)] border border-border rounded p-2 font-mono text-caption-sm text-text-2 placeholder:text-text-mute resize-none focus:outline-none focus:border-[rgba(255,255,255,0.20)]"
                        />
                        <button
                          onClick={() => handleFlag(expense.id)}
                          disabled={!flagInput[expense.id]?.trim()}
                          className={cn(
                            "flex items-center gap-1 font-mono text-micro uppercase tracking-wider transition-colors",
