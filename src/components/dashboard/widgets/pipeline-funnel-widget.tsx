...
} from "@/components/ops/segmented-picker";
...
export function PipelineFunnelWidget({
  ...
  const stages = useMemo(() => {
    ...
    const maxCount = Math.max(
      ...PIPELINE_STAGES.map(
        (s) => activeProjects.filter((p) => p.status === s.status).length
      ),
      1
    );

    return PIPELINE_STAGES.map((stage) => {
      const stageProjects = activeProjects.filter(
        (p) => p.status === stage.status
      );
  ...
  const stagePickerOptions = useMemo<SegmentedPickerOption[]>(() => {
    const opts: SegmentedPickerOption[] = [
      { value: "all", label: t("trend.all") ?? "All" },
    ];
    for (const stage of stages) {
      if (stage.count > 0) {
        opts.push({
          value: stage.status,
          label: `${t(stage.i18nKey)} (${stage.count})`,
        });
      }
    }
    return opts;
      ...
      <Card className="h-full">
        <div className="flex h-full flex-col px-3 py-2">
          ...
          <div className="flex flex-1 flex-col justify-center">
            <span
              className={`font-mono ${compact ? HERO_SIZE_CLASS.compact : HERO_SIZE_CLASS.expanded} font-bold leading-none text-text-mute`}
            >
              0
            </span>
            <span className="mt-1 font-mohave text-caption-sm text-text-mute">
  ...
  if (size === "xs") {
    return (
      <Card className="h-full">
        <div className="flex h-full flex-col pt-3" ref={ref}>
    ...
    return (
      <Card className="h-full p-0" ref={ref}>
        <div className="flex h-full p-3">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-[3px] px-3">
            {stages.map((stage, i) => (
          ...
          <button
            ...
            className="shrink-0 self-start rounded-sm p-0.5 transition-colors hover:bg-surface-hover"
          >
            <ArrowUpRight className="h-2.5 w-2.5 text-text-mute" />
          </button>
        </div>
      </Card>
    );
    ...
    <div className="flex flex-1 flex-col gap-[3px]">
      {stages.map((stage, i) => {
        ...
        return (
          ...
          >
            <div
              className="relative flex shrink-0 cursor-pointer items-center rounded-sm"
              style={{
              ...
              onClick={(e) => {
                ...
                if (showActions(size)) {
                  setStageFilter((prev) =>
                    prev === stage.status ? "all" : stage.status
                  );
            ...
            >
              {isWide && (
                <span
                  className="truncate px-2 font-mohave text-caption-sm"
            ...
            {!isWide && (
              <div className="ml-2 flex shrink-0 items-center gap-1 whitespace-nowrap">
  ...
  const filteredStageData =
    showActions(size) && stageFilter !== "all"
      ...
      <Card className="h-full p-0" ref={ref}>
        <div className="flex h-full flex-col p-3">
          ...
          >
            ...
            <TooltipRow
              label={t("pipelineFunnel.ofPipeline") ?? "Of pipeline"}
              value={`${tooltip.pct}%`}
            />
          </WidgetTooltip>
  ...
  return (
    <Card className="h-full p-0" ref={ref}>
      <div className="flex h-full flex-col p-3">
        ...
        <div className="mb-3">{renderHorizontalFunnel()}</div>
        ...
        <ScrollFade className="min-h-0 flex-1">
          ...
          {stageFilter === "all" && distribution.length > 0 && (
            <div className="flex flex-col gap-1.5">
              {distribution.map((d, i) => (
                <div
                  key={i}
                  className="-mx-1 flex cursor-pointer items-center justify-between rounded-sm px-1 py-1 transition-colors hover:bg-[rgba(255,255,255,0.03)]"
                  onClick={() => {
                    setStageFilter(
                      stages.find((s) => t(s.i18nKey) === d.label)?.status ??
                        "all"
                    );
                    setShowAllItems(false);
                  }}
                >
                  <div className="flex items-center gap-2">
                    <span
                      className="h-[6px] w-[6px] shrink-0 rounded-bar"
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-micro font-medium text-text">
                      {d.count}
                    </span>
                    <span className="w-[32px] text-right font-mono text-micro text-text-mute">
                      {d.pct}%
                    </span>
                  </div>
                </div>
          ...
          {filteredStageData && (
            <div className="flex flex-col">
              {(showAllItems
                ? filteredStageData.projects
                : filteredStageData.projects.slice(0, MAX_VISIBLE_ITEMS)
              ).map((p, i) => (
                <WidgetLineItem
                  key={p.id}
                  ...
                  secondary={[p.client?.name, p.address]
                    .filter(Boolean)
                    .join(" · ")}
