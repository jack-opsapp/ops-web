...
export function ProjectDeductionsTab({ project }: Props) {
  ...
  if (isLoading) {
    return (
      <p className="font-mono text-caption text-text-mute">
        Loading deductions...
      </p>
    );
  }
        ...
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="border-b border-border bg-[rgba(255,255,255,0.02)]">
              <th className="text-left px-3 py-2 font-mono text-caption-sm text-text-3 uppercase tracking-widest">
                Date
              </th>
              <th className="text-left px-3 py-2 font-mono text-caption-sm text-text-3 uppercase tracking-widest">
                Item
              ...
              <th className="text-right px-3 py-2 font-mono text-caption-sm text-text-3 uppercase tracking-widest">
                Qty
              </th>
              <th className="text-right px-3 py-2 font-mono text-caption-sm text-text-3 uppercase tracking-widest">
                Before → After
            ...
            {deductions.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-1.5 font-mono text-caption text-text-2 truncate max-w-[200px]">
                    {taskTitle ?? "—"}
                  </td>
                  ...
                  <td className="px-3 py-1.5 font-mono text-caption text-text-2">
                    {REASON_LABEL[d.reason]}
                  </td>
                  <td className="px-3 py-1.5 font-mono text-caption text-text-3 truncate max-w-[120px]">
                    {byName ?? "—"}
                  </td>
                </tr>
              );
