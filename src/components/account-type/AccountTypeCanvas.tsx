/**
 * AccountTypeCanvas — Canvas 2D particle field for the account type decision screen
 *
 * Adapted from ForcedChoiceResponse with OPS brand colors, higher particle count,
 * brighter particles, and reduced-motion support.
 *
...
const BLUE = { r: 89, g: 119, b: 148 }; // #6F94B0
...
export function AccountTypeCanvas({
  ...
  if (selected !== null) {
    ...
    if (idx >= 0 && selectedRef.current !== idx) {
  ...
  useEffect(() => {
    if (REDUCED_MOTION) {
      ...
      for (const p of particles) {
        const px = p.x * w;
        const py = p.y * h;
        const alpha = p.baseAlpha * edgeTaper(p.x, p.y);
        ctx.fillStyle = `rgba(${NEUTRAL.r}, ${NEUTRAL.g}, ${NEUTRAL.b}, ${alpha})`;
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const drawColor = isSelected ? nodeColor : NEUTRAL;
        const nodeSize = isSelected ? 12 : 7;
        const nodeAlpha = isSelected ? 0.95 : 0.4;
      ...
      for (let i = 0; i < Math.min(2, optionsRef.current.length); i++) {
        ...
        const nodeY = i === 0 ? leftY : rightY;
        const isSelected = selectedRef.current === i;
        ctx.font = '500 16px "Mohave", sans-serif';
        ctx.textAlign = "center";
        ctx.fillStyle = `rgba(255, 255, 255, ${isSelected ? 0.95 : 0.65})`;
    ...
    function draw(timestamp: number) {
      ...
      for (const p of particles) {
        if (sel >= 0) {
          const baseFlowSpeed =
            0.002 * (0.3 + selProgress * 0.7) * flowSpeedMult;
          ...
          const funnelStrength =
            (0.0002 + approachT * 0.003) * selProgress;

          p.vy += (selNode.ny - p.y) * funnelStrength;
          p.vy *= 0.92;
          p.vx +=
            (flowVx * (0.4 + approachT * 0.6) - p.vx) *
            (0.03 + selProgress * 0.06);

          if (distToNode < -0.1) {
            p.vy +=
              Math.sin(time * 1.5 + p.phase) * 0.0002 * (1 - approachT);
            p.vx +=
              Math.cos(time * 0.8 + p.phase * 2) * 0.00005 * (1 - approachT);
          } else {
            p.vy += Math.sin(time * 2 + p.phase) * 0.00003;
          }
          ...
          if (dist > 0.01) {
            ...
            p.vx += (-dy / dist) * 0.00008;
            p.vy += (dx / dist) * 0.00008;
          }
          p.vx += Math.sin(time * 0.15 + p.phase) * 0.00001;
          p.vy += Math.cos(time * 0.1 + p.phase * 1.3) * 0.00001;
          p.vx *= 0.985;
          p.vy *= 0.985;
          p.x += p.vx;
          p.y += p.vy;
          ...
          p.vx += Math.sin(time * 0.15 + p.phase) * 0.00003;
          p.vy += Math.cos(time * 0.1 + p.phase * 1.3) * 0.00003;
          p.vx *= 0.99;
          p.vy *= 0.99;
          p.x += p.vx;
          p.y += p.vy;
        ...
        if (sel >= 0) {
          ...
          alpha = Math.max(
            p.baseAlpha * 0.5,
            p.baseAlpha * 0.7 + proximity * selProgress * 0.5
          );
        } else if (hoverIdx >= 0) {
          const hovNode = hoverIdx === 0 ? LEFT_NODE : RIGHT_NODE;
          ...
          cb = NEUTRAL.b;
          alpha =
            p.baseAlpha + Math.sin(time * 0.5 + p.phase) * 0.03;
        }
