...
const EXTERNAL_EXPENSE_CONTEXT_RE =
  /\b(?:truck|engine|vehicle|car|medical|rent|mortgage|tax(?:es)?|insurance|personal debt|credit card)\b/i;
const DEAL_DOCUMENT_CONTEXT_RE =
  /\b(?:quote|estimate|proposal|offer|contract|scope of work)\b/i;
...
export function extractCommercialDealPriceMatches(
  ...
  for (const match of body.matchAll(PRICE_RE)) {
    ...
    if (
      ...
      (EXTERNAL_EXPENSE_CONTEXT_RE.test(segment) &&
        !DEAL_DOCUMENT_CONTEXT_RE.test(segment)) ||
      (/\b(?:deposit|payment)\b/i.test(segment) && !/\btotal\b/i.test(segment))
    ) {
      continue;
    }
