...
export const ESTIMATE_STATUS_COLORS: Record<EstimateStatus, string> = {
  ...
  [EstimateStatus.Accepted]: "#9DB582",
  [EstimateStatus.Rejected]: "#B58289",
  [EstimateStatus.Expired]: "#C4A868",
...
export const INVOICE_STATUS_COLORS: Record<InvoiceStatus, string> = {
  ...
  [InvoiceStatus.Partial]: "#C4A868",
  [InvoiceStatus.Paid]: "#9DB582",
  [InvoiceStatus.Overdue]: "#B58289",
  [InvoiceStatus.Void]: "#6B7280",
};

// ─── Financial Entity Interfaces ─────────────────────────────────────────────
...
/** Accounting connection per company */
...
// ─── Financial Create/Update Types ───────────────────────────────────────────

export type CreateProduct = Omit<Product, "id" | "deletedAt" | "createdAt" | "updatedAt">;
export type UpdateProduct = Partial<CreateProduct> & { id: string };

export type CreateLineItem = Omit<LineItem, "id" | "product">;
export type UpdateLineItem = Partial<CreateLineItem> & { id: string };

export type CreateEstimate = Omit<
  Estimate,
  "id" | "estimateNumber" | "deletedAt" | "createdAt" | "updatedAt" | "lineItems" | "client" | "project"
>;
export type UpdateEstimate = Partial<CreateEstimate> & { id: string };

export type CreateInvoice = Omit<
  Invoice,
  "id" | "invoiceNumber" | "deletedAt" | "createdAt" | "updatedAt" | "lineItems" | "payments" | "client" | "project" | "estimate"
>;
export type UpdateInvoice = Partial<CreateInvoice> & { id: string };

export type CreatePayment = Omit<Payment, "id" | "deletedAt" | "createdAt" | "updatedAt">;
...
export function calculateLineItemTax(amount: number, taxRate: number): number {
  return Math.round(amount * (taxRate / 100) * 100) / 100;
}

/** Calculate line item discount */
export function calculateLineItemDiscount(amount: number, discountPercent: number): number {
...
export function isEstimateExpired(estimate: Pick<Estimate, "expirationDate" | "status">): boolean {
...
export function isInvoiceOverdue(invoice: Pick<Invoice, "dueDate" | "status" | "balance">): boolean {
  if (invoice.status === InvoiceStatus.Paid || invoice.status === InvoiceStatus.Void) {
    return false;
  }
  if (!invoice.dueDate) return false;
  return new Date() > invoice.dueDate && invoice.balance > 0;
}

/** Get payment terms as days */
export function getPaymentTermsDays(terms: string): number {
  switch (terms) {
    case "Due on Receipt": return 0;
    case "Net 7": return 7;
    case "Net 10": return 10;
