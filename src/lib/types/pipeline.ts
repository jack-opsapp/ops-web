...
export interface LineItem {
  ...
  type: LineItemType;
  /** Legacy task-type display/id snapshot. */
  taskTypeId: string | null;
  /** Modern FK to task_types.id, snapshotted when the product is selected. */
  taskTypeRef: string | null;
  ...
  unitId: string | null;
  unitPrice: number;
  /** Resolved per-unit price after configured option modifiers. */
  ...
  /** Signed configuration snapshot; never recomputed from a later Product edit. */
  configuredOptions: Record<string, unknown> | null;
  /** Human-readable signed configuration, e.g. "Color: Cobblestone". */
