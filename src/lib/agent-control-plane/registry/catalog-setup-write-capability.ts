...
export const PREPARE_SET_CATALOG_PRICING_CAPABILITY_DEFINITION = Object.freeze({
  ...
  description:
    "Prepare the selling price of one catalogue family or one catalogue variant for exact operator approval inside OPS. Sale price resolves as the variant override when set, otherwise the family default, so aim a family ref at the number every variant should inherit and a variant ref at the one that should differ. A family ref writes the family default and never writes a variant override; the preview lists every variant the new default moves and every variant that will keep their own price, marking any whose own price equals the new default. A variant ref writes that variant's override only when the amount differs from the family default: an amount equal to the family default leaves the variant inheriting, clearing any override it carried. An explicit null clears the price at the level the ref names: clearing a family default leaves every variant carrying no override of its own with no price at all, and the preview lists each one. The amount is a decimal string, amounts carry at most two decimals for CAD/USD, and the currency must equal the company's own currency. A request that resolves to the price already on file, at the level already on file, is refused rather than staged. This tool never writes cost. Costs are recorded as supplier cost profiles with prepare_set_supplier_cost. No stock moves, no message is sent and no accounting sync is enqueued.",
...
export const PREPARE_SET_SUPPLIER_COST_CAPABILITY_DEFINITION = Object.freeze({
  ...
  writeFamily: "catalog_setup_write",
  description:
    "Prepare one supplier cost profile on one catalogue variant for exact operator approval inside OPS. Profiles are keyed by profile_key, not by a supplier record, so a variant can carry several rate cards at once — a standard rate and a negotiated one, say — and a variant that carries any profile carries exactly one default. Setting is_default demotes the current default in the same write; a request that would leave the variant with profiles and no default is refused. Writing a key whose profile was previously removed brings that row back, reported as revived rather than created. The cost is a decimal string, amounts carry at most two decimals for CAD/USD, and the currency must equal the company's own currency. OPS keeps two cost models and this tool keeps them together: whenever the profile that ends up as the variant's default changes, or that default's cost changes, the variant's catalogue cost is mirrored to that number in the same write, at the level the family already uses — set on the variant, unless the family cost already equals it, in which case the variant goes on inheriting it. This tool never writes the family cost. The preview reports the catalogue cost and its origin on both sides. activation_rule and source are small objects OPS stores verbatim; OPS adds its own provenance under the reserved ops key. The preview lists every profile the variant has, on both sides, so the default flip is visible rather than inferred. No price moves, no stock moves, no message is sent and no accounting sync is enqueued.",
...
export const PREPARE_CREATE_CATALOG_VARIANT_CAPABILITY_DEFINITION =
  Object.freeze({
    ...
    writeFamily: "catalog_setup_write",
    description:
      "Prepare one new variant on an existing catalogue family for exact operator approval inside OPS. Answer every non-deleted option on the family exactly once; a value set that already exists is refused. Selling price resolves as the variant price override, else the family default, so a price is required when the family has no default; amounts carry at most two decimals for CAD/USD. Thresholds are whole units, as OPS stores them. A price equal to the family default, or a level equal to the one the family or its category already answers, is inherited rather than set on the variant, so a later family change still reaches it; the preview reports each value with its origin. An opening quantity is recorded as a stock unit and a receive stock event, never as a silent count. Nothing is written, no message is sent and no accounting sync is enqueued during preparation.",
