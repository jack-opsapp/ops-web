...
import { calculateDeckGeometryFromSourceJson, parseDeckGeometrySource } from "../deck-geometry-calculator";
  ...
  describe(`${producer} native optional stair destination`, () => {
    for (const mode of ["absent", "null"] as const) {
      it(`preserves the real connection and measured stairs with ${mode} lower edge`, () => {
        ...
        expect(result.topology.connections[0]).toMatchObject({
          kind: "level_stair", lower_edge_ref: null,
          stair: { width: { state: "recorded", inches: 48 }, stringer: { state: "authoritative", rise_inches: 48, tread_count: 7, run_per_tread_inches: 10 } },
        });
        expect(result.full_precision.stair_railing_inches).toBeCloseTo(2 * Math.hypot(48, 70), 10);
        expect(result.measurements.flat_railing_linear_feet).toEqual({ state: "authoritative", value: 0, warning: null });
    ...
    for (const invalid of ["missing-edge", "upper-e1", "", 42, {}]) {
      it(`rejects an invalid named lower edge ${JSON.stringify(invalid)}`, () => {
        ...
        expect(() => parseDeckGeometrySource(JSON.stringify(drawing))).toThrow("DECK_GEOMETRY_REFERENCE_INVALID");
      });
    }
  });
}
