...
import {
  ...
  type JobDiscoverySnapshot,
} from "./job-discovery-repository";

function safeMessage(code: JobDiscoveryReadError["code"]): string {
  if (code === "NOT_FOUND") return "Job discovery results were not found.";
  if (code === "STALE_CONTEXT") {
    return "Job discovery results changed during pagination.";
  }
  if (code === "TEMPORARILY_UNAVAILABLE") {
    return "Job discovery is temporarily unavailable.";
  }
  if (code === "RESULT_TOO_LARGE") {
    return "Job discovery result is too large.";
  }
  return "Job discovery could not be read.";
}
...
export class JobDiscoveryReadError extends Error {
  readonly code:
    | "NOT_FOUND"
    | "STALE_CONTEXT"
  ...
  constructor(input: {
    readonly code: JobDiscoveryReadError["code"];
    readonly requestId: string;
    readonly retryable: boolean;
    readonly currentSourceVersion?: SourceVersion | null;
    ...
    this.name = "JobDiscoveryReadError";
...
function mapRepositoryError(
  ...
  if (error instanceof JobDiscoveryRepositoryError) {
    if (error.code === "JOB_DISCOVERY_NOT_FOUND") {
      throw new JobDiscoveryReadError({
        code: "NOT_FOUND",
    ...
    if (error.code === "JOB_DISCOVERY_STALE" && error.currentSourceVersion) {
      throw new JobDiscoveryReadError({
        code: "STALE_CONTEXT",
        requestId: authorization.actorContext.requestId,
        retryable: true,
    ...
    if (error.code === "JOB_DISCOVERY_READ_FAILED") {
      throw new JobDiscoveryReadError({
...
function resultCandidate(input: {
  ...
  readonly snapshot: JobDiscoverySnapshot;
...
function parseResult(candidate: unknown): JobDiscoveryResult {
  return deepFreeze(JobDiscoveryResultSchema.parse(candidate));
}
...
function reduceToBudget(input: {
  ...
  let best: JobDiscoveryResult | null = null;
  let bestCount = -1;
  while (lower <= upper) {
...
export async function searchJobs(input: {
  readonly authorization: AuthorizedJobDiscoveryRead;
  readonly repository: JobDiscoveryRepository;
  readonly signal?: AbortSignal;
  readonly now?: () => Date;
}): Promise<JobDiscoveryResult> {
  const authorization = input.authorization;
  if (!isAuthorizedJobDiscoveryRead(authorization)) {
    throw new JobDiscoveryReadError({
      code: "INTERNAL",
  ...
  if (!isTrustedJobDiscoveryRepository(repository)) {
    throw new JobDiscoveryReadError({
      code: "INTERNAL",
      requestId: authorization.actorContext.requestId,
      retryable: false,
  ...
  let snapshot: JobDiscoverySnapshot;
  try {
  ...
  try {
    ...
    if (error instanceof JobDiscoveryReadError) throw error;
    throw new JobDiscoveryReadError({
      code: "INTERNAL",
      requestId: authorization.actorContext.requestId,
      retryable: false,
