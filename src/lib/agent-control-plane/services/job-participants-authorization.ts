...
import type { ParsedJobParticipantsInput } from "@/lib/agent-control-plane/contracts/communication";
import {
...
declare const AUTHORIZED_JOB_PARTICIPANTS_READ: unique symbol;

export interface AuthorizedJobParticipantsRead {
  readonly [AUTHORIZED_JOB_PARTICIPANTS_READ]: true;
  ...
  readonly query: ParsedJobParticipantsInput;
}
...
export function authorizeJobParticipantsRead(input: {
  readonly authorizations:
    | AuthorizedCapability
    | readonly AuthorizedCapability[];
  readonly rawInput: unknown;
}): AuthorizedJobParticipantsRead {
  ...
  if (
    ...
    throw authorizationInternal(
      "unknown-request",
      "job_participants_capability_untrusted"
    );
  }
  ...
  if (authorizations.some((proof) => proof.actorContext !== actorContext)) {
    throw authorizationInternal(
      ...
      "job_participants_actor_mismatch"
    );
  }
  ...
  try {
    ...
    throw authorizationInternal(
      actorContext.requestId,
      "job_participants_input_untrusted"
    );
  }
  if (
    ...
    throw authorizationInternal(
      actorContext.requestId,
      "job_participants_variant_mismatch"
    );
  }
  ...
  for (const variant of resolved.variants) {
    ...
    if (matchIndex < 0) {
      throw authorizationInternal(
        actorContext.requestId,
        "job_participants_capability_identity_mismatch"
      );
    }
  ...
  const query = resolved.parsedInput as ParsedJobParticipantsInput;
  ...
  if (
    ...
    throw authorizationInternal(
      actorContext.requestId,
      "job_participants_oauth_scope_unproven"
    );
  }
  ...
  const needsTaskLinkage =
    query.purpose === "schedule" || query.purpose === "assignment";
  if (
    !clientsScope ||
    !inboxScope ||
    (query.job_ref.kind === "opportunity" && !pipelineScope) ||
    (query.job_ref.kind === "project" && !projectsScope) ||
    (needsTaskLinkage && (!projectsScope || !tasksScope))
  ) {
    throw authorizationInternal(
      actorContext.requestId,
      "job_participants_permission_union_invalid"
    );
  }
  ...
  PROOFS.add(proof);
  return Object.freeze(proof) as unknown as AuthorizedJobParticipantsRead;
}

export function isAuthorizedJobParticipantsRead(
  value: unknown
): value is AuthorizedJobParticipantsRead {
