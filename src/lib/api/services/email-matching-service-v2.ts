...
function assertLookupSucceeded(
  ...
  if (error) {
    throw new Error(
      `[email-matching] ${context} failed: ${error.message ?? "unknown error"}`
    );
  }
}
...
export const EmailMatchingServiceV2 = {
  ...
  async match(
    ...
    const literalEmail = escapeIlikeLiteral(normalizedEmail);
    ...
    const { data: exactClient, error: exactClientError } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    assertLookupSucceeded("exact client lookup", exactClientError);
    ...
    // Check sub-clients
    const { data: exactSub, error: exactSubError } = await supabase
      .from("sub_clients")
      .select("id, client_id")
      .eq("company_id", companyId)
      .ilike("email", literalEmail)
      ...
      .limit(1)
      .maybeSingle();
    assertLookupSucceeded("exact sub-client lookup", exactSubError);
    ...
    if (domain && !PUBLIC_EMAIL_DOMAINS.has(domain)) {
      ...
      const safeDomain = escapeIlikeLiteral(domain);

      const { data: domainClients, error: domainClientsError } = await supabase
        .from("clients")
        .select("id, name, email")
        .eq("company_id", companyId)
        .ilike("email", `%@${safeDomain}`)
        .is("deleted_at", null);
      assertLookupSucceeded("domain client lookup", domainClientsError);

      const { data: domainSubs, error: domainSubsError } = await supabase
        .from("sub_clients")
        .select("id, client_id, email")
        .eq("company_id", companyId)
        .ilike("email", `%@${safeDomain}`)
        .is("deleted_at", null);
      assertLookupSucceeded("domain sub-client lookup", domainSubsError);

      const allDomainMatches = [
        ...(domainClients || []).map((c: { id: string }) => ({
          clientId: c.id,
          type: "client",
        })),
        ...(domainSubs || []).map((s: { client_id: string }) => ({
      ...
      const uniqueClientIds = [
        ...new Set(allDomainMatches.map((m) => m.clientId)),
      ];
    ...
    if (options?.threadId && options?.connectionId) {
      const { data: threadLinks, error: threadLinksError } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      assertLookupSucceeded("provider thread lookup", threadLinksError);
      ...
      if (threadLinks?.opportunity_id) {
        const { data: opportunity, error: opportunityError } = await supabase
          ...
          .is("deleted_at", null)
          .maybeSingle();
        assertLookupSucceeded(
          "provider thread opportunity lookup",
          opportunityError
        );
