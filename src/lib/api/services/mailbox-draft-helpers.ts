 ...
 *  Keys match the profileType vocabulary emitted by maybeAutoGenerateDraft's
 *  subject heuristic (sync-engine.ts). Sensitive/non-client categories
 *  (warranty_claim, vendor_ordering, subtrade_coordination) intentionally stay
 *  draft_on_request (resolved to that by the missing-key fallback in
 *  sync-engine.ts: `categoryAutonomy[profileType] || "draft_on_request"`). */
