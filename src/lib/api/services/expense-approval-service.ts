...
function mapBatchFromDb(row: Record<string, unknown>): ExpenseBatch {
  return {
    ...
    scopeProjectId: (row.scope_project_id as string) ?? null,
...
function mapExpenseFromDb(row: Record<string, unknown>): ExpenseLineItem {
  ...
  return {
    ...
    recurringReimbursementId: (row.recurring_reimbursement_id as string) ?? null,
    recurringPeriod: (row.recurring_period as string) ?? null,
    createdAt: row.created_at as string,
