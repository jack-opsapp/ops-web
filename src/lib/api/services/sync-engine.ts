...
import {
  ...
  type EmailIngestionRecoveryFailureDisposition,
  type EmailIngestionRecoveryWorkerResult,
} from "./email-ingestion-recovery-worker";
import type { EmailExactMessageRecoveryReparentRepairInput } from "./email-exact-message-recovery-service";
...
interface CreateOpportunityTitleOptions {
  ...
  mailboxAssignment?: {
    ...
    ingestionSource: "email_sync" | "email_recovery";
...
interface EmailIngestionExecutionPolicy {
  /** Provider writes are forbidden for exact, already-fetched recovery input. */
  providerMutationsDisabled: boolean;
  /** Recovery must not enqueue thread classification or downstream autonomy. */
  suppressThreadState: boolean;
  assignmentIngestionSource: "email_sync" | "email_recovery";
}

const NORMAL_EMAIL_INGESTION_POLICY: EmailIngestionExecutionPolicy = {
  providerMutationsDisabled: false,
  suppressThreadState: false,
  assignmentIngestionSource: "email_sync",
};

const EXACT_MESSAGE_RECOVERY_POLICY: EmailIngestionExecutionPolicy = {
  providerMutationsDisabled: true,
  suppressThreadState: true,
  assignmentIngestionSource: "email_recovery",
};
...
function mailboxAssignmentContext(
  connection: EmailConnection,
  ...
  return {
    ...
    ingestionSource: executionPolicy.assignmentIngestionSource,
    providerMutationsDisabled: executionPolicy.providerMutationsDisabled,
  };
}
...
interface EmailIngestionRecoveryRow {
  id: unknown;
  company_id: unknown;
...
function requiredRecoveryString(value: unknown, field: string): string {
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`Email ingestion recovery row is missing ${field}`);
  }
  return value;
}

function nullableRecoveryString(value: unknown, field: string): string | null {
  if (value === null || value === undefined) return null;
  if (typeof value !== "string") {
    throw new Error(`Email ingestion recovery row has invalid ${field}`);
  }
  return value;
}

function mapEmailIngestionRecoveryRow(
  value: unknown
): ClaimedEmailIngestionRecovery & { status: string } {
  const candidate = Array.isArray(value) ? value[0] : value;
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error("Email ingestion recovery RPC returned no row");
  }
  ...
  const attempts =
    typeof row.attempts === "number" ? row.attempts : Number(row.attempts);
  if (!Number.isSafeInteger(attempts) || attempts < 0) {
    throw new Error("Email ingestion recovery row has invalid attempts");
  }
  return {
    id: requiredRecoveryString(row.id, "id"),
    companyId: requiredRecoveryString(row.company_id, "company_id"),
    connectionId: requiredRecoveryString(row.connection_id, "connection_id"),
    kind,
    providerThreadId: requiredRecoveryString(
      row.provider_thread_id,
      "provider_thread_id"
    ),
    providerMessageId: nullableRecoveryString(
      row.provider_message_id,
      "provider_message_id"
    ),
    providerLabelId: nullableRecoveryString(
...
function emailIngestionRecoveryRows(
  value: unknown
): ClaimedEmailIngestionRecovery[] {
  if (value === null || value === undefined) return [];
  if (!Array.isArray(value)) {
    return [mapEmailIngestionRecoveryRow(value)];
  }
  return value.map((row) => mapEmailIngestionRecoveryRow(row));
}

function recoveryRpcBoolean(value: unknown): boolean {
  if (typeof value === "boolean") return value;
  if (Array.isArray(value)) return value[0] === true;
  return false;
}

function recoveryFailureDisposition(
  value: unknown
): EmailIngestionRecoveryFailureDisposition {
  const candidate = Array.isArray(value) ? value[0] : value;
  if (
    candidate === "retrying" ||
    candidate === "failed" ||
  ...
  throw new Error(
    "Email ingestion recovery failure RPC returned an invalid disposition"
  );
}
...
async function enqueueEmailIngestionRecovery(input: {
  ...
  if (error) {
    throw new CronDatabaseOperationError(
      `[sync-engine] email ingestion recovery enqueue failed: ${error.message}`,
      { cause: error }
    );
  }
  return mapEmailIngestionRecoveryRow(data);
}

async function claimEmailIngestionRecoveryById(input: {
  queueId: string;
  holder: string;
}): Promise<(ClaimedEmailIngestionRecovery & { status: string }) | null> {
  const { data, error } = await requireSupabase().rpc(
  ...
  if (error) {
    throw new CronDatabaseOperationError(
      `[sync-engine] email ingestion recovery direct claim failed: ${error.message}`,
      { cause: error }
    );
  }
...
async function completeEmailIngestionRecovery(input: {
  ...
  if (error) {
    throw new CronDatabaseOperationError(
      `[sync-engine] email ingestion recovery completion failed: ${error.message}`,
      { cause: error }
    );
  }
  return recoveryRpcBoolean(data);
}

async function failEmailIngestionRecovery(input: {
  queueId: string;
  holder: string;
  error: string;
  ...
  if (error) {
    throw new CronDatabaseOperationError(
      `[sync-engine] email ingestion recovery failure persistence failed: ${error.message}`,
      { cause: error }
    );
  }
  return recoveryFailureDisposition(data);
}
...
async function applyPersonalMailboxAssignment(params: {
  ...
  if (params.mailboxAssignment.connectionType !== "individual") {
    throw new LifecyclePersistenceError(
      "[sync-engine] company mailbox assignment requires atomic opportunity creation"
    );
  }
  try {
    const result = await assignPersonalMailboxLead(
      {
        ...
        ingestionSource: params.mailboxAssignment.ingestionSource,
        providerMutationsDisabled:
          params.mailboxAssignment.providerMutationsDisabled,
      },
      requireSupabase()
    );
...
interface AtomicCompanyMailboxOpportunityResult {
  opportunityId: string;
  clientId: string;
  assignedTo: string | null;
  assignmentVersion: number;
  created: boolean;
  assignmentOutcome: "assigned" | "owner_missing" | "owner_ineligible" | null;
  assignmentEventId: string | null;
  promptCount: number;
}

function atomicCompanyMailboxResult(
  value: unknown
): AtomicCompanyMailboxOpportunityResult {
  ...
  const opportunity =
    result.opportunity &&
    typeof result.opportunity === "object" &&
    !Array.isArray(result.opportunity)
  ...
  const assignedTo =
    typeof opportunity?.assigned_to === "string"
      ...
      : null;
  const assignmentVersion = Number(opportunity?.assignment_version);
  ...
  if (
    ...
    throw new Error(
      "[sync-engine] atomic company mailbox opportunity returned invalid identity"
    );
  }

  if (!created) {
    if (result.reason !== "source_key_exists" || assignment !== null) {
      throw new Error(
        "[sync-engine] existing company mailbox opportunity returned invalid retry state"
      );
    }
    return {
      opportunityId,
  ...
  if (
    ...
    throw new Error(
      "[sync-engine] atomic company mailbox opportunity returned invalid assignment state"
    );
  }
  if (
    result.reason !==
      (outcome === "assigned" ? "created_assigned" : "created_prompted") ||
    (outcome === "assigned" &&
      (!assignedTo ||
        assignmentVersion < 1 ||
        !eventId ||
    ...
    throw new Error(
      "[sync-engine] atomic company mailbox opportunity returned inconsistent assignment state"
    );
  }
...
async function createAtomicCompanyMailboxOpportunity(params: {
  opportunity: Record<string, unknown>;
  providerThreadId: string;
  mailboxAssignment: NonNullable<
    CreateOpportunityTitleOptions["mailboxAssignment"]
  >;
}): Promise<AtomicCompanyMailboxOpportunityResult> {
  const { data, error } = await requireSupabase().rpc(
    "create_company_mailbox_email_opportunity_as_system",
    {
      p_connection_id: params.mailboxAssignment.connectionId,
      p_opportunity: params.opportunity,
      p_provider_thread_id: params.providerThreadId,
      p_ingestion_source: params.mailboxAssignment.ingestionSource,
      p_provider_mutations_disabled:
        params.mailboxAssignment.providerMutationsDisabled,
    }
  );
  if (error) {
    throw new Error(
      `[sync-engine] atomic company mailbox opportunity failed: ${error.message ?? "unknown error"}`
    );
  }
  return atomicCompanyMailboxResult(data);
}
...
async function createOpportunity(
  ...
  if (titleOptions.mailboxAssignment?.connectionType === "company") {
    if (!sourceKey || !email.threadId) {
      throw new Error(
        "[sync-engine] company mailbox opportunity requires deterministic provider identity"
      );
    }
    const atomicOpportunityInsert: Record<string, unknown> = {
      ...opportunityInsert,
    };
    // The guarded RPC owns the immutable email source. Do not forward even the
    // equivalent enrichment value through its strict user-data allowlist.
    delete atomicOpportunityInsert.source;
    const atomic = await createAtomicCompanyMailboxOpportunity({
      opportunity: atomicOpportunityInsert,
      providerThreadId: email.threadId,
      mailboxAssignment: titleOptions.mailboxAssignment,
    });
    if (!atomic.created) {
      // The atomic insert may have committed while a later, idempotent
      // provenance write failed. Re-run that semantic tail on the authoritative
      // source-key winner, but never turn a dedupe into an assignment backfill.
      if (titleOptions.enrichmentFacts) {
        await writeFieldProvenance({
          supabase,
          companyId,
          opportunityId: atomic.opportunityId,
          clientId: null,
      ...
      return {
        id: atomic.opportunityId,
        clientId: atomic.clientId,
        created: false,
      };
    }
    if (atomic.assignmentOutcome !== "assigned") {
      console.warn("[email-ingest] company-mailbox lead left unassigned", {
        opportunityId: atomic.opportunityId,
        connectionId: titleOptions.mailboxAssignment.connectionId,
        reason: atomic.assignmentOutcome,
        promptsEnqueued: atomic.promptCount,
      });
    }
    data = {
      id: atomic.opportunityId,
      client_id: atomic.clientId,
      assigned_to: atomic.assignedTo,
      assignment_version: atomic.assignmentVersion,
    };
  } else {
  ...
  if (error) {
    if ((error as { code?: string }).code === "23505" && sourceKey) {
      ...
      if (existing?.id && existing.client_id) {
        if (titleOptions.mailboxAssignment?.connectionType === "individual") {
 ...
 * NULL -> target CAS and the first correspondence projection; the ordinary
 * recorder is replayed afterwards to repair lifecycle high-water state.
 */
async function createOrAdoptInboundActivity(input: {
  email: NormalizedEmail;
  connection: EmailConnection;
  opportunityId: string | null;
  extra?: ActivityPersistenceOptions;
  ...
  if (!input.existingOrphanActivity) {
    ...
    if (!created) return { persisted: false, created: false, activityId: null };
  ...
  // A review/non-lead outcome deliberately leaves the durable orphan
  // untouched. Never create a second activity for the same provider identity.
  if (!input.opportunityId) {
    return {
      persisted: false,
      created: false,
      activityId: input.existingOrphanActivity.id ?? null,
    };
  }
  if (!input.existingOrphanActivity.id || !input.syncLockOwner) {
    throw new LifecyclePersistenceError(
      "[sync-engine] exact orphan adoption identity is incomplete"
    );
  }

  const profile = input.connection.syncFilters as Partial<SyncProfile> | null;
  const fromEmail = extractSenderEmail(input.email.from);
  const toEmails = input.email.to.map(extractSenderEmail);
  const ccEmails = input.email.cc.map(extractSenderEmail);
  const classification = classifyOpportunityCorrespondence({
    direction: "inbound",
    providerThreadId: input.email.threadId,
    providerMessageId: input.email.id,
    fromEmail,
  ...
  const { data, error } = await requireSupabase().rpc(
    "adopt_orphan_email_activity_with_payload_guard_as_system",
    {
      ...
      p_company_id: input.connection.companyId,
      p_connection_id: input.connection.id,
      p_sync_lock_owner: input.syncLockOwner,
      p_activity_id: input.existingOrphanActivity.id,
      p_target_opportunity_id: input.opportunityId,
      p_provider_thread_id: input.email.threadId,
      p_provider_message_id: input.email.id,
      p_ingestion_source: input.executionPolicy.assignmentIngestionSource,
      p_match_confidence: input.extra?.matchConfidence ?? "ai",
      ...
      p_occurred_at: input.email.date.toISOString(),
      p_subject: input.email.subject,
  ...
  if (
    ...
    adoption.opportunity_id !== input.opportunityId
  ) {
    throw new LifecyclePersistenceError(
      "[sync-engine] orphan inbound adoption returned a conflicting identity"
    );
  }

  // The adoption RPC inserted/projected the correspondence event in the same
  // transaction. Replay the canonical service to idempotently advance the
  // lifecycle high-water state that intentionally lives outside that RPC.
  await recordActivityCorrespondenceEvent(
    input.email,
    input.connection,
    input.opportunityId,
    input.existingOrphanActivity.id,
    "inbound"
  );
  await persistDeterministicEmailThreadState(
    input.email,
...
async function updateCorrespondenceCounts(
  ...
  // recordActivityCorrespondenceEvent has already inserted the immutable event
  // and projected its counters in one database transaction. Read the locked
  // projection that transaction committed; never reopen the old two-RPC seam
  // that could strand a durable event with pending counters. A retry records
  // the same provider identity idempotently (and repairs any legacy pending
  // row) before reaching this read again.
  const { data: row, error: projectionReadError } = await supabase
    .from("opportunities")
    .select(
      "correspondence_count, inbound_count, outbound_count, stage, stage_manually_set, assignment_version, last_inbound_at, last_outbound_at, last_message_direction"
    )
    .eq("id", opportunityId)
    .eq("company_id", connection.companyId)
    .is("deleted_at", null)
  ...
  if (projectionReadError || !row) {
    throw new Error(
      `[sync-engine] correspondence projection read failed for ${opportunityId}: ${projectionReadError?.message ?? "opportunity not found"}`
    );
  }
...
async function applyLabel(
  ...
  if (executionPolicy.providerMutationsDisabled || !connection.opsLabelId)
    return;
  if (!syncLockOwner) {
    throw new LifecyclePersistenceError(
      `[sync-engine] provider label write lacks the mailbox lease for ${messageId}`
    );
  }
  const recovery = await enqueueEmailIngestionRecovery({
    companyId: connection.companyId,
    connectionId: connection.id,
    kind: "provider_label_apply",
    providerThreadId: threadId,
  ...
  const claimed = await claimEmailIngestionRecoveryById({
    queueId: recovery.id,
    holder: syncLockOwner,
  });
  ...
  try {
    ...
    await failEmailIngestionRecovery({
      queueId: recovery.id,
      holder: syncLockOwner,
      error: err instanceof Error ? err.message : String(err),
    });
    console.error(
      `[sync-engine] Deferred provider label recovery for thread ${threadId}:`,
      err
    );
    return;
  }
  await providerLockCheckpoint();
  const completed = await completeEmailIngestionRecovery({
    queueId: recovery.id,
    holder: syncLockOwner,
  ...
  if (!completed) {
    throw new LifecyclePersistenceError(
      `[sync-engine] provider label recovery completion changed for ${messageId}`
    );
  }
...
async function findExistingProviderActivity(
  ...
  // Legacy activities predate email_connection_id. Read-side thread evidence
  // may identify a candidate, but it never authorizes the write: the guarded
  // claim RPC below must lock the row and re-prove its exact correspondence
  // event before assigning the mailbox identity.
  ...
  if (matchingLegacy[0]) {
    ...
    const { data: claimed, error: claimError } = await supabase.rpc(
      "claim_legacy_email_activity_connection_as_system",
      {
        ...
        p_activity_id: legacyActivity.id,
    ...
    return { ...legacyActivity, email_connection_id: connection.id };
  }
  if (hasUnknownLegacy) {
    throw new Error(
...
interface StableDiscoveredEmail {
  email: NormalizedEmail;
  direction: "inbound" | "outbound";
...
function requirePersistedActivityDirection(
  activity: ExistingProviderActivity
): "inbound" | "outbound" {
  if (activity.direction !== "inbound" && activity.direction !== "outbound") {
    throw new LifecyclePersistenceError(
      `[sync-engine] persisted activity ${activity.id ?? "unknown"} has invalid direction`
    );
  }
...
async function resolveStableDiscoveredEmails(
...
async function processInboundEmail(
  ...
  const existingActivity =
    preloadedExistingActivity === undefined
      ? await findExistingProviderActivity(
          supabase,
          connection,
          email.id,
          email.threadId
  ...
  if (executionPolicy.suppressThreadState) {
    return {
      email,
      effectiveEmail,
      routingIdentity,
...
async function persistAIClassifiedUnmatchedInbound(input: {
  ...
  await input.providerLockCheckpoint(true);
  ...
  for (const classified of [...deterministicLeads, ...aiResult.classifiedLeads]) {
    ...
    try {
      ...
      const classifiedStageReview = coerceAIStageForOpportunityPersistence(
        classified.stage,
      ...
      if (relationshipDecision.action === "link") {
        ...
        const opportunity = await createOpportunity(
          ...
          classifiedStageReview.stage,
          {
            ...
            unsafe: syncTitleUnsafeIdentity(input.connection, input.profile),
            enrichmentFacts: deterministicFacts,
            sourceKey: routingIdentity.sourceKey,
            mailboxAssignment: mailboxAssignmentContext(
              input.connection,
      ...
      const subClientIdentity = subClientIdentityFromFacts(deterministicFacts);
      if (
        matchResult.action === "create_subclient" &&
        clientId &&
...
/**
 * Durably defer Step-5 lead classification by exact provider message before
 * the mailbox cursor advances. The service-only recovery queue covers both
 * ordinary thread rows and message-scoped forwarded contact forms. The legacy
 * `email_threads.lead_scan_pending_at` marker remains as a compatibility signal
 * for non-contact-form rows, but it is no longer the source of durability.
 *
 * Queue persistence is authoritative: a failure throws and withholds the
 * cursor. The compatibility marker remains best-effort because the exact
 * message queue is already durable and deduplicated.
 */
async function markUnmatchedThreadsPendingLeadScan(
  ...
  try {
    ...
    if (isDatabasePressureError(err)) throw err;
    console.error(
      "[sync-engine] failed to write compatibility lead-scan marker (exact-message recovery remains queued):",
...
export const SyncEngine = {
  /**
   * Persist one exact provider message that was already fetched by the guarded
   * recovery reader. This deliberately reuses canonical matching, enrichment,
   * assignment, stage, and summary logic while disabling every provider-side
   * mutation and leaving the mailbox cursor untouched.
   */
  async ingestExactInboundMessageForRecovery(input: {
    actorUserId: string;
    companyId: string;
    connectionId: string;
    email: NormalizedEmail;
  }): Promise<SyncCycleResult> {
    const connection = await EmailService.getConnection(input.connectionId);
    ...
    if (connection.companyId !== input.companyId) {
      return {
        ...emptyResult(),
        errors: ["Connection belongs to another company"],
      };
    }

    const { data: ingestAuthorized, error: ingestAuthorizationError } =
    ...
    if (ingestAuthorizationError || ingestAuthorized !== true) {
      return {
        ...emptyResult(),
        errors: ["Recovery actor lacks mailbox or pipeline ingest authority"],
      };
    }
    ...
    try {
      ...
      const direction = existingRecoveryActivity
        ? requirePersistedActivityDirection(existingRecoveryActivity)
        : resolvePersistedEmailDirection(recoveryEmail, ingestionOperator);
      if (direction !== "inbound") {
        throw new LifecyclePersistenceError(
          "[sync-engine] exact recovery message is not a canonical inbound message"
        );
      }
      ...
      if (
        ...
        throw new LifecyclePersistenceError(
          "[sync-engine] exact recovery message is blocked by current email filters"
        );
      }
      ...
      if (!activity?.opportunity_id) {
        throw new LifecyclePersistenceError(
          "[sync-engine] exact recovery message has no canonical opportunity"
        );
      }
      ...
      if (routingIdentity.mayInheritProviderThread) {
        ...
        // durable internal thread is still required by the guarded commercial
        // outcome evaluator. Upsert only; do not schedule Phase C.
        const { threadRow } = await EmailThreadService.upsertFromEmail({
          companyId: connection.companyId,
          connectionId: connection.id,
        ...
        await EmailThreadService.refreshSummaryOnly(threadRow);
      }
      ...
      if (
        ...
        throw new LifecyclePersistenceError(
          `[sync-engine] exact recovery summary refresh incomplete: requested=${summaryRefresh.requested} written=${summaryRefresh.written} feature_disabled=${summaryRefresh.skippedFeatureDisabled} failures=${summaryRefresh.failed
      ...
      console.error(
        `[sync-engine] Error recovering exact message ${input.email.id}:`,
        error
      );
      result.errors.push(
        error instanceof Error ? error.message : "Unknown error"
  ...
  /**
   * Finish the deterministic projections after the guarded exact-message RPC
   * moves one already-ingested message. This method has no provider transport
   * dependency: it re-authorizes both lead scopes, proves exact target
   * ownership, evaluates the target's complete commercial history, and
   * refreshes both affected summaries under the mailbox sync lock.
   */
  async repairExactReparentedMessageForRecovery(
    ...
    if (
      ...
      connection.companyId !== input.companyId
    ) {
      throw new LifecyclePersistenceError(
        "[sync-engine] exact reparent repair connection is not active for this company"
      );
    }
    if (
      input.message.id !== input.entry.providerMessageId ||
      input.message.threadId !== input.entry.providerThreadId
    ) {
      throw new LifecyclePersistenceError(
        "[sync-engine] exact reparent repair provider identity changed"
      );
    }

    const supabase = requireSupabase();
    const { data: inboxAuthorized, error: inboxAuthorizationError } =
      await supabase.rpc("authorize_email_inbox_action_as_system", {
        p_actor_user_id: input.actorUserId,
        p_connection_id: input.connectionId,
        p_opportunity_id: null,
        p_action: "view",
      });
    if (inboxAuthorizationError || inboxAuthorized !== true) {
      throw new LifecyclePersistenceError(
        "[sync-engine] exact reparent repair mailbox authorization changed"
      );
    }
    ...
    for (const opportunityId of [
      ...
      if (error || authorized !== true) {
        throw new LifecyclePersistenceError(
          `[sync-engine] exact reparent repair lead authorization changed for ${opportunityId}`
        );
      }
    }
    ...
    if (!syncLockOwner) {
      throw new LifecyclePersistenceError(
        "[sync-engine] exact reparent repair mailbox is busy"
      );
    }
    const renewSyncLeaseIfNeeded = createEmailConnectionSyncLockRenewer({
      connectionId: connection.id,
    ...
    try {
      ...
      if (
        eventError ||
        !event ||
        event.direction !== activity.direction ||
        event.party_role !== "customer" ||
        event.is_meaningful !== true ||
        event.opportunity_projection_applied !== true
      ) {
        throw new LifecyclePersistenceError(
          `[sync-engine] exact reparent repair correspondence changed: ${eventError?.message ?? "row not found or not projected customer evidence"}`
        );
      }
      ...
      if (
        summaryRefresh.skippedFeatureDisabled ||
        summaryRefresh.failed.length > 0 ||
        summaryRefresh.requested !== 2 ||
        summaryRefresh.written !== 2
      ) {
        throw new LifecyclePersistenceError(
          `[sync-engine] exact reparent summary refresh incomplete: requested=${summaryRefresh.requested} written=${summaryRefresh.written} feature_disabled=${summaryRefresh.skippedFeatureDisabled} failures=${summaryRefresh.failed
            .map((failure) => `${failure.opportunityId}: ${failure.error}`)
            .join("; ")}`
        );
      }
  ...
  async runSync(connectionId: string): Promise<SyncCycleResult> {
    ...
    try {
      ...
      // immutable persisted direction for retries and use current roster
      // identity only for provider messages OPS has never ingested.
      ...
      const stableDiscoveredEmails = await resolveStableDiscoveredEmails(
        discoveredEmails,
        connection,
        directionIdentity
      );
      const stableDiscoveryByMessageId = new Map(
        stableDiscoveredEmails.map((entry) => [entry.email.id, entry])
      );
      ...
      try {
        ...
        for (const rawEmail of [...inboxEmails, ...sentEmails]) {
          ...
          const evaluationKey = !routing.mayInheritProviderThread
            ? routing.sourceKey
          ...
          if (!activeLeadTargets.has(evaluationKey)) {
            activeLeadTargets.set(
              evaluationKey,
              !routing.mayInheritProviderThread
  ...
  /**
   * Drain exact-message classification deferrals and idempotent provider-label
   * writes. Claims are tenant-filtered by the cron's current subscription gate,
   * then reauthorized inside the physical-mailbox lease immediately before any
   * provider access. Successful retries reuse the canonical ingest and label
   * paths; no alternate lifecycle implementation exists here.
   */
  async retryPendingIngestionRecovery(options: {
    companyIds: string[];
    limit?: number;
  }): Promise<EmailIngestionRecoveryWorkerResult> {
    const supabase = requireSupabase();
    const worker = new EmailIngestionRecoveryWorker({
      async claim(input) {
        ...
        if (error) {
          throw new CronDatabaseOperationError(
            `[sync-engine] email ingestion recovery claim failed: ${error.message}`,
            { cause: error }
          );
        }
        return emailIngestionRecoveryRows(data);
      },
      ...
      async reauthorize(input) {
        ...
        if (error) {
          throw new CronDatabaseOperationError(
            `[sync-engine] email ingestion recovery reauthorization failed: ${error.message}`,
            { cause: error }
          );
        }
        return recoveryRpcBoolean(data);
      ...
      async runWithMailboxLease(input) {
        ...
        if (!syncLockOwner) return { acquired: false };
        const renewSyncLeaseIfNeeded = createEmailConnectionSyncLockRenewer({
        ...
        try {
          return {
            acquired: true,
            value: await input.run(renewSyncLeaseIfNeeded, syncLockOwner),
          };
        } finally {
          clearOperatorIdentityForSyncCycle(input.connectionId);
          await renewSyncLeaseIfNeeded.stop().catch(() => {});
          await releaseEmailConnectionSyncLock(
            input.connectionId,
            syncLockOwner,
      ...
      async applyProviderLabel(input) {
        if (!input.job.providerLabelId) {
          throw new Error("EMAIL_INGESTION_RECOVERY_LABEL_ID_MISSING");
        }
        await input.providerLockCheckpoint();
        const provider = EmailService.getProvider(input.connection);
        await provider.applyLabel(
          input.job.providerThreadId,
          input.job.providerLabelId
        );
        await input.providerLockCheckpoint();
      },

      async recoverLeadClassification(input) {
        const providerMessageId = input.job.providerMessageId;
        if (!providerMessageId) {
          throw new Error("EMAIL_INGESTION_RECOVERY_MESSAGE_ID_MISSING");
        }
        const profile: SyncProfile = {
          ...(input.connection.syncFilters as SyncProfile),
        };
        profile.internalPhones = [
          ...(profile.internalPhones ?? []),
          ...(await loadInternalPhonesForCompany(input.connection.companyId)),
        ];
        const authoritativeOperator = await refreshOperatorIdentityForSyncCycle(
          input.connection
        );
        const directionIdentity = syncIngestionOperatorIdentity(
          input.connection,
        ...
        if (legacyError) {
          throw new CronDatabaseOperationError(
            `[sync-engine] recovery compatibility marker read failed: ${legacyError.message}`,
            { cause: legacyError }
          );
        }
        for (const row of legacyRows ?? []) {
          await clearLeadScanPendingMarker(supabase, row.id as string);
        }
      ...
      fail: failEmailIngestionRecovery,
      workerId: randomUUID,
    });
        ...
        for (const thread of threads) {
          ...
          try {
            ...
            const unmatchedContext = await processInboundEmail(
              ...
              syncLockOwner,
              false,
              latestInbound.existingActivity
            );
            if (!unmatchedContext) {
              await clearLeadScanPendingMarker(supabase, thread.id);
              outcome.cleared += 1;
