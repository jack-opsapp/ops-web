...
async function processInboundEmail(
  ...
  if (threadLink && threadLink.length > 0) {
    ...
    maybeAutoGenerateDraft(effectiveEmail, connection, threadLink[0].opportunity_id)
    ...
    if (draftResult.draftHistoryId) {
      try {
        ...
        if (existing?.mailbox_draft_id) {
          ...
          if (!inboxUiEnabled && connection.userId) {
            try {
              const { data: priorMailboxRows } = await supabase
                .from("ai_draft_history")
                .select("id")
                .eq("connection_id", connection.id)
                .not("mailbox_draft_id", "is", null)
                .limit(1);
              const isFirstMailboxDraft =
                !priorMailboxRows || priorMailboxRows.length === 0;
              if (isFirstMailboxDraft) {
                const mailboxName =
                ...
                await supabase.from("notifications").insert({
                  ...
                  title: "Replies, pre-drafted",
                  body: `OPS now drafts your replies straight into your ${mailboxName} drafts. Read it, send it.`,
                  is_read: false,
                  persistent: false,
                  action_url: "/settings?tab=integrations",
