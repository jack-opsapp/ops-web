  ...
  if (canPush) {
    // ── Push Clients ────────────────────────────────────────────────────────
    {
      ...
      for (const client of clients ?? []) {
        try {
          const { qbId } = await QuickBooksSyncService.pushClient(
            accessToken,
            realmId,
            providerEnvironment,
            {
    ...
    // ── Push Invoices ───────────────────────────────────────────────────────
    {
      ...
      for (const inv of invoices ?? []) {
        const clientJoin = inv.clients as unknown as
          | { qb_id: string | null }
          | Array<{ qb_id: string | null }>;
        const clientQbId = Array.isArray(clientJoin)
          ? clientJoin[0]?.qb_id
          : clientJoin?.qb_id;
        if (!clientQbId) continue;
        try {
          const { qbId } = await QuickBooksSyncService.pushInvoice(
            accessToken,
            realmId,
            providerEnvironment,
            {
              ...
              lineItems: [
                { description: "Invoice sync", amount: inv.total ?? 0 },
              ],
    ...
    // ── Push Estimates ──────────────────────────────────────────────────────
    {
      ...
      const { data: estimates } = await supabase
        .from("estimates")
        .select(
          "id, client_id, total, expiration_date, qb_id, clients!inner(qb_id)"
        )
      ...
      for (const est of estimates ?? []) {
        const clientJoin = est.clients as unknown as
          | { qb_id: string | null }
          | Array<{ qb_id: string | null }>;
        const clientQbId = Array.isArray(clientJoin)
          ? clientJoin[0]?.qb_id
        ...
        if (!clientQbId) continue;
        try {
          const { qbId } = await QuickBooksSyncService.pushEstimate(
            accessToken,
            realmId,
            providerEnvironment,
            {
              customerRef: clientQbId,
              lineItems: [
                { description: "Estimate sync", amount: est.total ?? 0 },
              ],
              expirationDate: est.expiration_date ?? undefined,
    ...
    // ── Push Payments ───────────────────────────────────────────────────────
    {
      ...
      const { data: payments } = await supabase
        .from("payments")
        .select(
          "id, client_id, amount, payment_date, qb_id, invoice_id, clients!inner(qb_id), invoices(qb_id)"
        )
        .eq("company_id", companyId)
        .or(lastSyncAt ? `updated_at.gte.${lastSyncAt}` : "id.not.is.null");
      ...
      for (const pmt of payments ?? []) {
        const clientJoin = pmt.clients as unknown as
          | { qb_id: string | null }
          | Array<{ qb_id: string | null }>;
        const clientQbId = Array.isArray(clientJoin)
          ? clientJoin[0]?.qb_id
          : clientJoin?.qb_id;
        if (!clientQbId) continue;
        const invoiceJoin = pmt.invoices as unknown as
          | { qb_id: string | null }
          | Array<{ qb_id: string | null }>
          | null;
        const invoiceQbId = invoiceJoin
          ? Array.isArray(invoiceJoin)
            ? invoiceJoin[0]?.qb_id
            : invoiceJoin?.qb_id
          : undefined;
        try {
          const { qbId } = await QuickBooksSyncService.pushPayment(
            accessToken,
            realmId,
            providerEnvironment,
            {
      ...
      try {
        ...
        for (const c of pulled) {
          try {
            ...
            errors.push(
              `Pull upsert client ${c.qbId}: ${(err as Error).message}`
            );
          }
        }
        results.push({
      ...
      try {
        ...
        for (const inv of pulled) {
          try {
            ...
            errors.push(
              `Pull upsert invoice ${inv.qbId}: ${(err as Error).message}`
            );
          }
        }
        results.push({
...
async function syncSage(
  ...
  const { accessToken } = await AccountingTokenService.getValidToken(
    supabase,
    connectionId
  );
  const results: SyncResult[] = [];
  ...
  if (canPush) {
    ...
    // ── Push Invoices ───────────────────────────────────────────────────────
    {
      ...
      const { data: invoices } = await supabase
        .from("invoices")
        .select(
          "id, client_id, total, due_date, sage_id, clients!inner(sage_id)"
        )
        .eq("company_id", companyId)
        .or(lastSyncAt ? `updated_at.gte.${lastSyncAt}` : "id.not.is.null");
      ...
      for (const inv of invoices ?? []) {
        const clientJoin = inv.clients as unknown as
          | { sage_id: string | null }
          | Array<{ sage_id: string | null }>;
        const clientSageId = Array.isArray(clientJoin)
          ? clientJoin[0]?.sage_id
          : clientJoin?.sage_id;
    ...
    // ── Push Estimates ──────────────────────────────────────────────────────
    {
      ...
      const { data: estimates } = await supabase
        .from("estimates")
        .select(
          "id, client_id, total, expiration_date, sage_id, clients!inner(sage_id)"
        )
        .eq("distribution_hold", false)
        .eq("company_id", companyId)
        .or(lastSyncAt ? `updated_at.gte.${lastSyncAt}` : "id.not.is.null");
      ...
      for (const est of estimates ?? []) {
        ...
        try {
          const { sageId } = await SageSyncService.pushEstimate(accessToken, {
            ...
            lineItems: [
              {
                description: "Estimate sync",
                quantity: 1,
                unitPrice: est.total ?? 0,
              },
            ],
            expiryDate: est.expiration_date ?? undefined,
    ...
    // ── Push Payments ───────────────────────────────────────────────────────
    {
      ...
      const { data: payments } = await supabase
        .from("payments")
        .select(
          "id, client_id, amount, payment_date, sage_id, invoice_id, clients!inner(sage_id), invoices(sage_id)"
        )
        .eq("company_id", companyId)
        .or(lastSyncAt ? `updated_at.gte.${lastSyncAt}` : "id.not.is.null");
  ...
  if (canPull) {
    // ── Pull Clients (upsert into local DB) ───────────────────────────────
    {
      const errors: string[] = [];
      try {
        const pulled = await SageSyncService.pullClients(
          accessToken,
          lastSyncAt ?? undefined
        );
        let count = 0;
        for (const c of pulled) {
          try {
            ...
            errors.push(
              `Pull upsert client ${c.sageId}: ${(err as Error).message}`
            );
          }
        }
        results.push({
    ...
    // ── Pull Invoices (upsert into local DB) ──────────────────────────────
    {
      const errors: string[] = [];
      try {
        const pulled = await SageSyncService.pullInvoices(
          accessToken,
          lastSyncAt ?? undefined
        );
        let count = 0;
        for (const inv of pulled) {
          try {
            ...
            errors.push(
              `Pull upsert invoice ${inv.sageId}: ${(err as Error).message}`
            );
          }
        }
        results.push({
...
export async function runSyncForConnection(
  ...
  if (!direction) {
    ...
    if (connErr || !conn) {
      throw new Error(
        `Connection ${connectionId} not found for direction check`
      );
    }
