...
describe("AccountingSyncQueueService", () => {
  ...
  it("schedules retry with exponential backoff", async () => {
    ...
    await service.scheduleRetry(queueRow({ attempts: 2, maxAttempts: 5 }), "rate limited");
    ...
    const patch = db.update.mock.calls[0][0] as { run_after: string };
    expect(Date.parse(patch.run_after)).toBeGreaterThan(Date.now());
  });
  ...
  it("blocks a row when max attempts are exhausted", async () => {
    ...
    await service.scheduleRetry(queueRow({ attempts: 5, maxAttempts: 5 }), "validation failed");
