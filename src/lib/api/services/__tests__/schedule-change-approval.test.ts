...
function receipt(replayed = false) {
  return {
    ok: true,
    effect: "task_schedule_and_crew_updated_inside_ops",
    run_id: runId,
    action_id: actionId,
    ...
    readback: resultFixture().proposal.tasks.map(task => ({ task_id: task.task_id, schedule: task.after })),
...
function fakeSupabase(finalAction = action()) {
  ...
  Object.assign(builder, {
    ...
    in: () => builder,
    limit: async () => ({
      data: [{ id: actionId, action_type: "approve_schedule_change" }],
      error: null,
    }),
...
describe("schedule change approval boundary", () => {
  beforeEach(() => vi.clearAllMocks());

  it("commits only the displayed exact schedule change and validates its readback receipt", async () => {
    ...
    expect(fixture.rpc).toHaveBeenCalledWith(
      "commit_agent_schedule_change_as_actor",
      {
  ...
  it("rejects edits, autonomous execution, and invalid effect receipts", async () => {
    ...
    ).rejects.toThrow("Schedule changes require operator approval");
    expect(executeAutonomousMock).not.toHaveBeenCalled();
  ...
  it("records rejection through the coherent database decision", async () => {
    ...
    expect(fixture.rpc).toHaveBeenCalledWith(
      "reject_agent_schedule_change_as_actor",
      {
        p_actor_user_id: userId,
        p_company_id: companyId,
        p_action_id: actionId,
  ...
  it("cannot be swept into bulk approval", async () => {
    ...
    ).rejects.toThrow("Schedule changes must be approved one proposal at a time");
    expect(fixture.rpc).not.toHaveBeenCalled();
  });
});

describe("schedule change queue privacy", () => {
  ...
  it("returns full content only when the database independently allows the exact actor/action", async () => {
    ...
    expect(rpc).toHaveBeenCalledWith(
      "filter_agent_schedule_change_actions_as_actor",
