...
import { receiptFixture } from "@/lib/agent-control-plane/services/catalog-authoring/__tests__/fixtures";
...
function action(status = "executed") {
  return {
    ...
    user_id: userId,
    action_type: "approve_catalog_changes",
    action_data: { change_set_id: changeSetId, preview_sha256: previewSha256 },
    context_summary: "Catalog save ready for exact review",
    context_source: "control_room",
    source_id: "catalog:" + changeSetId,
    confidence: 1,
    priority: "high",
    status,
...
function fakeSupabase(finalAction = action()) {
  ...
  Object.assign(builder, {
    ...
    in: () => builder,
    limit: async () => ({
      data: [{ id: actionId, action_type: "approve_catalog_changes" }],
      error: null,
    }),
...
describe("catalog approval boundary", () => {
  ...
  it("preserves the exact approval and asks for a retry when catalog writers are busy", async () => {
    const fixture = fakeSupabase();
    fixture.rpc.mockResolvedValue({
      data: null,
  ...
  it.each([
    ...
    async (args) => {
      ...
      ).rejects.toThrow("Review the current catalog preview");
      expect(fixture.rpc).not.toHaveBeenCalled();
    }
  );

  it("saves only the displayed exact catalog and validates its readback receipt", async () => {
    const fixture = fakeSupabase();
    requireSupabaseMock.mockReturnValue(fixture.client);
    ...
    expect(fixture.rpc).toHaveBeenCalledWith(
      "commit_catalog_changes_as_actor",
      {
  ...
  it("rejects edits, autonomous execution, and invalid effect receipts", async () => {
    ...
    ).rejects.toThrow("Review the current catalog preview before approving");
    expect(fixture.rpc).not.toHaveBeenCalled();

    await expect(
      ApprovalQueueService.executeAutonomousAction(actionId)
    ).rejects.toThrow("Catalog changes require exact operator approval");
    expect(executeAutonomousMock).not.toHaveBeenCalled();
...
describe("catalog queue privacy", () => {
  ...
  it("returns full content only when the database independently allows the exact actor/action", async () => {
    ...
    expect(rpc).toHaveBeenCalledWith("filter_catalog_actions_as_actor", {
      p_actor: userId,
      p_company: companyId,
      p_actions: [actionId],
    });
