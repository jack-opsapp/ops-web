...
export const ApprovalQueueService = {
  ...
  async getQueue(
    ...
    const scheduleChanges = rows.filter(row => row.action_type === "approve_schedule_change");
    if (scheduleChanges.length && actorUserId) {
      const visibility = await supabase.rpc("filter_agent_schedule_change_actions_as_actor" as never, {
        p_actor: actorUserId, p_company: companyId, p_actions: scheduleChanges.map(row => row.id),
      } as never);
      if (!visibility.error) {
        const parsed = z.array(z.uuid()).max(200).safeParse(visibility.data);
    ...
    if (customerMessages.length && actorUserId) {
      const visibility = await supabase.rpc(
        "filter_agent_customer_message_actions_as_actor" as never,
        {
          p_actor: actorUserId,
          p_company: companyId,
          p_actions: customerMessages.map((row) => row.id),
        } as never
      );
      if (!visibility.error) {
        const parsed = z.array(z.uuid()).max(200).safeParse(visibility.data);
        if (parsed.success) readableMessageIds = new Set(parsed.data);
      }
    }
    const actions = rows.map((row) =>
      mapFromDb(
        (row.action_type === "approve_schedule_change" && !readableScheduleIds.has(String(row.id))) ||
        (row.action_type === "approve_customer_update" &&
          ...
          ? {
              ...
              action_data: {},
              context_summary:
                "Details are unavailable. Reject this request to clear it.",
  ...
  async approveAction(
    ...
    if (actionIdentity.action_type === "send_customer_follow_up") {
      ...
      if (
        ...
        throw new Error("Review the exact customer reply before approving");
      }
      const args = {
        p_actor_user_id: userId,
        p_company_id: companyId,
      ...
      let approval = await supabase.rpc(
        "approve_agent_customer_message_as_actor" as never,
        args as never
      );
      if (approval.error || !approval.data) {
        approval = await supabase.rpc(
          "approve_agent_customer_message_as_actor" as never,
          args as never
        );
      }
      if (approval.error || !approval.data) {
        throw new Error(
          "Customer reply approval could not be reconciled. Reload before retrying."
        );
      }
      const approvalReceipt = CustomerMessageReceiptSchema.parse(approval.data);
      if (
        approvalReceipt.action_id !== actionId ||
        approvalReceipt.change_set_id !== args.p_change_set_id ||
        approvalReceipt.preview_sha256 !== args.p_preview_sha256
      ) {
        throw new Error("Customer reply approval receipt is invalid");
      }
      ...
      try {
        ...
        if (boundary !== "pre_provider_retryable") {
          throw new Error(
            "The customer reply provider outcome is being reconciled. Reload for its durable status."
          );
        }
        const message =
          error instanceof Error ? error.message : "Unknown error";
        throw new Error(`Customer reply could not start: ${message}`);
      }
      ...
      const { data: final, error } = await supabase
        ...
        .eq("user_id", userId)
        .single();
      if (error || !final || !final.execution_result) {
        throw new Error("Customer reply status readback is unavailable");
      }
      ...
      if (
        ...
        throw new Error("Customer reply status receipt is invalid");
      }
      return mapFromDb(final);
    }
  ...
  async rejectAction(
    ...
    if (actionIdentity.action_type === "send_customer_follow_up") {
      ...
      if (decision.error || !decision.data) {
        throw new Error("Customer reply cancellation could not be recorded");
      }
      const receipt = CustomerMessageReceiptSchema.parse(decision.data);
      if (
        receipt.action_id !== actionId ||
        receipt.state !== "cancelled_before_send"
      ) {
        throw new Error("Customer reply cancellation receipt is invalid");
      }
      const { data: final, error: finalError } = await supabase
        .from("agent_actions")
        .select("*")
        ...
        .single();
      if (finalError || !final || final.status !== "rejected") {
        throw new Error("Customer reply cancellation readback failed");
      }
      CustomerMessageReceiptSchema.parse(final.execution_result);
      return mapFromDb(final);
    }
    if (actionIdentity.action_type === "approve_dispatch_confirmation_task") {
      const decision = await supabase.rpc(
  ...
  async bulkApprove(
    ...
    if (exactConfirmations && exactConfirmations.length > 0) {
      ...
      if (exactConfirmations[0]?.action_type === "approve_schedule_change")
        throw new Error("Schedule changes must be approved one proposal at a time");
