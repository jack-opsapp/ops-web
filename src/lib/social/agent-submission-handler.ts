...
export function createSocialSubmissionHandler(
  ...
  return async function handleSocialSubmission(
    request: NextRequest
    ...
    if (
      !providedSecret ||
      !secureTokenEquals(providedSecret, configuredSecret)
    ) {
      return NextResponse.json(
    ...
    if (Number.isFinite(declaredBytes) && declaredBytes > 100_000) {
      return NextResponse.json(
        {
          error: "Social package exceeds 100 KB",
          code: "SOCIAL_PACKAGE_TOO_LARGE",
        },
        { status: 413 }
      );
    }
