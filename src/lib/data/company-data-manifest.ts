 ...
 *
 * ── The two-way completeness rule ──────────────────────────────────────────
 * `tests/integration/company-data-manifest.test.ts` diffs this manifest against
 * the checked-in generated schema, `src/lib/types/database.types.ts`:
 *
 *   1. Every table in the types' `Tables:` block that has a `company_id`
 *      column MUST appear here. A new company-scoped table therefore cannot
 *      reach production unclassified.
 *   2. Every table named here MUST exist in the types' `Tables:` block, except
 *      the documented `UNTYPED_TABLE_ALLOWLIST` — backend tables the checked-in
 *      types have not caught up to.
 *
 * Tests cannot reach production, so **regenerating `database.types.ts` is what
 * forces a new table to be consciously classified.** After any migration that
 * adds a company-scoped table, regenerate the types; the completeness test then
 * fails until the table is given a strategy and an export decision here.
 *
 ...
 *
 * At the time of writing that is 180 company-scoped base tables, 31
 * parent-scoped children (29 direct + `catalog_option_values` and
 * `product_option_values` one level deeper), and the `companies` row itself.
 *
...
export const PARENT_SCOPED_DATA: readonly ParentScopedEntry[] = [
  ...
  },
  {
    ...
    export: false,
    reason:
      "Part of the OPS SPEC sales ledger — contract acceptance, Stripe payments, refunds and the abuse blocklist. It is OPS's own commercial record of a completed sale, not the trades company's operating data, and it must survive account closure for financial, legal and anti-abuse reasons. Its deeper children (spec_scope_documents, spec_payments, spec_change_orders, spec_retainers, spec_referrals, spec_satisfaction_ratings, spec_support_tickets) hang off spec_projects and are retained with it.",
  },
  {
