...
interface ExpenseBatchPopoverStoreState {
  popovers: Map<string, ExpenseBatchPopoverState>;
  nextZIndex: number;

  openPopover: (batchId: string, screenPosition: { x: number; y: number }, title: string, color: string) => void;
  ...
  setActiveTab: (id: string, tab: ExpenseBatchPopoverTab) => void;
}
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, ExpenseBatchPopoverState>
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useExpenseBatchPopoverStore = create<ExpenseBatchPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (batchId, screenPosition, title, color) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(batchId);
