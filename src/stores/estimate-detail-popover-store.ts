...
// ── Types ──
export type EstimatePopoverTab = "overview" | "activity";

export interface EstimateDetailPopoverState {
  id: string;
  title: string;
  color: string;
  ...
  zIndex: number;
  isMinimized: boolean;
  activeTab: EstimatePopoverTab;
}

interface EstimateDetailPopoverStoreState {
  popovers: Map<string, EstimateDetailPopoverState>;
  nextZIndex: number;

  openPopover: (estimateId: string, screenPosition: { x: number; y: number }, title: string, color: string) => void;
  ...
  setActiveTab: (id: string, tab: EstimatePopoverTab) => void;
}
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, EstimateDetailPopoverState>
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useEstimateDetailPopoverStore = create<EstimateDetailPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (estimateId, screenPosition, title, color) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(estimateId);

      if (existing) {
        const updated = new Map(popovers);
        updated.set(estimateId, {
          ...existing,
          isMinimized: false,
          zIndex: nextZIndex,
        });
