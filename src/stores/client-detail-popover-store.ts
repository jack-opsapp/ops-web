...
// ── Types ──
export type ClientPopoverTab = "overview" | "projects" | "financial" | "activity";

export interface ClientDetailPopoverState {
  id: string;
  title: string;
  color: string;
  ...
  zIndex: number;
  isMinimized: boolean;
  activeTab: ClientPopoverTab;
}

interface ClientDetailPopoverStoreState {
  popovers: Map<string, ClientDetailPopoverState>;
  nextZIndex: number;

  openPopover: (clientId: string, screenPosition: { x: number; y: number }, title: string, color: string) => void;
  ...
  setActiveTab: (id: string, tab: ClientPopoverTab) => void;
}
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, ClientDetailPopoverState>
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useClientDetailPopoverStore = create<ClientDetailPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (clientId, screenPosition, title, color) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(clientId);
