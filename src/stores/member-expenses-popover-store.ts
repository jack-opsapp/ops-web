...
// ── Constants ──
export const POPOVER_DEFAULT_WIDTH = 380;
export const POPOVER_DEFAULT_HEIGHT = 460;
export const POPOVER_MIN_WIDTH = 320;
export const POPOVER_MIN_HEIGHT = 280;
...
export interface MemberExpensesPopoverState {
  /** User ID of the team member */
  id: string;
  /** Display name */
  title: string;
  /** Accent color for the title bar indicator */
  color: string;
...
interface MemberExpensesPopoverStoreState {
  popovers: Map<string, MemberExpensesPopoverState>;
  nextZIndex: number;

  openPopover: (memberId: string, screenPosition: { x: number; y: number }, title: string, color: string) => void;
...
function findNonOverlappingPosition(
  baseX: number,
  baseY: number,
  existing: Map<string, MemberExpensesPopoverState>,
): { x: number; y: number } {
  let x = baseX;
  let y = baseY;
...
export const useMemberExpensesPopoverStore = create<MemberExpensesPopoverStoreState>()(
  (set, get) => ({
    popovers: new Map(),
    nextZIndex: POPOVER_Z_BASE,

    openPopover: (memberId, screenPosition, title, color) => {
      const { popovers, nextZIndex } = get();
      const existing = popovers.get(memberId);
