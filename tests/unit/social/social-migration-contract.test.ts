...
function loadMigration(): string {
  ...
  expect(
    filename,
    "social publishing migration must be generated"
  ).toBeDefined();
  return readFileSync(
...
describe("social publishing migration contract", () => {
  ...
  it("claims due rows atomically with a fixed security boundary", () => {
    ...
    expect(sql).toContain(
      "revoke all on function public.claim_due_social_posts"
    );
    expect(sql).toContain(
      "grant execute on function public.claim_due_social_posts"
    );
    ...
    expect(sql).toContain(
      "revoke all on function public.claim_social_post_by_id"
    );
    expect(sql).toContain(
    ...
    expect(sql).toContain(
      "revoke all on function public.record_social_publish_stage"
    );
  });
  ...
  it("recovers only pre-publish leases and quarantines uncertain outcomes", () => {
    ...
    expect(sql).toContain(
      "social_post.publish_stage in ('claimed', 'container_ready')"
    );
    expect(sql).toContain(
      "social_post.publish_stage in ('publish_requested', 'publish_succeeded')"
    );
    ...
    expect(sql).toContain(
      "social_post.attempt_count >= social_post.max_attempts"
    );
    expect(sql).toContain("recovery_notification_pending = true");
    expect(sql).toContain(
      "social_post.updated_at <= now() - interval '15 minutes'"
    );
  ...
  it("leases and atomically delivers durable recovery notifications", () => {
    ...
    expect(sql).toContain(
      "grant execute on function public.claim_social_recovery_notifications"
    );
    expect(sql).toContain(
      "grant execute on function public.deliver_social_recovery_notification"
    );
    expect(sql).toContain(
    ...
    expect(sql).toContain(
      "notification.dedupe_key = 'social-recovery:' || new.id::text"
    );
    expect(sql).toContain(
