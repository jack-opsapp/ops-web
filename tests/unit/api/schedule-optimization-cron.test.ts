...
const mocks = vi.hoisted(() => ({
  ...
  suggestScheduleOptimizations: vi.fn(),
}));
...
vi.mock("@/lib/api/services/schedule-optimization-service", () => ({
  ScheduleOptimizationService: {
    suggestScheduleOptimizations: mocks.suggestScheduleOptimizations,
  },
}));
...
import { GET } from "@/app/api/cron/schedule-optimization/route";
import {
...
function request() {
  return new NextRequest("https://ops.test/api/cron/schedule-optimization", {
    headers: { authorization: "Bearer cron-secret" },
  });
}

describe("schedule optimization cron", () => {
  beforeEach(() => {
    ...
    mocks.getCompanyManagerUserIds.mockRejectedValue(
      new Error("column companies.schedule_settings does not exist")
    );
  });
  ...
  it("fails the workload instead of reporting a company schema error as success", async () => {
    ...
    expect(await response.json()).toMatchObject({
      ok: false,
      error: "Schedule optimization failed for 3 of 3 companies",
      retry: {
        status: "scheduled",
        scheduled: [
      ...
      results: [
        {
          ...
          error: "column companies.schedule_settings does not exist",
        },
        {
          companyId: "company-2",
          today: null,
