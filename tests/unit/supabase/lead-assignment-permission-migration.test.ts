...
describe("lead assignment permission migration", () => {
  ...
  it("parses the incoming role owner before validating the final role assignment", () => {
    const guard = body("private.guard_user_roles_final_state");
    const declarationPosition = guard.search(/v_new_user_id\s+uuid\s*;/i);
    const assignmentPosition = guard.search(/v_new_user_id\s*:=/i);

    expect(declarationPosition).toBeGreaterThan(-1);
    expect(assignmentPosition).toBeGreaterThan(-1);
    expect(declarationPosition).toBeLessThan(assignmentPosition);
  });
