...
-- Task 19 forward-only replay proof. Both generated migrations are
...
begin
  if pg_catalog.current_setting('server_version_num')::integer < 170000
    ...
    raise exception 'task19_forward_ledger requires PostgreSQL 17';
  end if;
  ...
  where not trigger_row.tgisinternal
    ...
    and function_row.proname = 'bump_agent_purchasing_source_revision';
  if v_trigger_count <> 8 then
    raise exception 'task19_forward_ledger trigger count:%', v_trigger_count;
  end if;
end;
$task19_forward_ledger$;

do $task19_replay_source$
declare
  v_index text;
begin
  if pg_catalog.to_regprocedure(
    ...
    raise exception 'task19_replay_source function missing';
  end if;
  foreach v_index in array array[
    'idx_catalog_orders_agent_delivery_v1',
    'idx_catalog_order_items_agent_order_id_v1'
  ]::text[] loop
    if not exists (
      select 1
      ...
      raise exception 'task19_replay_source index invalid:%', v_index;
    end if;
  end loop;
end;
$task19_replay_source$;

do $task19_replay_reads$
declare
  v_signature text;
begin
  foreach v_signature in array array[
    ...
    if pg_catalog.to_regprocedure(v_signature) is null then
      raise exception 'task19_replay_reads function missing:%', v_signature;
    end if;
  end loop;
end;
$task19_replay_reads$;
