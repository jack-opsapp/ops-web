...
grant execute on function private.agent_p2_team_summary_v1(
...
begin
  if pg_catalog.has_function_privilege(
    ...
    raise exception 'agent_team_replay_failed: acl_drift';
  end if;

  select pg_catalog.count(*) into v_trigger_count
  from pg_catalog.pg_trigger trigger_row
  where trigger_row.tgname = 'users_bump_agent_team_revision'
    and not trigger_row.tgisinternal
    and trigger_row.tgenabled = 'O'
    and trigger_row.tgtype = 29;
  if v_trigger_count <> 1 then
    raise exception 'agent_team_replay_failed: trigger_drift';
  end if;
end;
$replay_contract$;
